...
- Examples: `Server Resources`, `Posts per Month`, `Search Results for "…"`, `Information Stored in Your Browser`, and `Meet in the Middle`.
...
`docs/project/ui/` documents **laikit UI as a public, general-purpose React component library developed by lailai**, for any developer to use. Introductions lead with capabilities, design principles, installation, and customization. Do not define its purpose around the author's own websites, list those sites as the core audience, or frame the library as an internal component extraction. Write integration and dependency-upgrade guidance so a reader outside those projects can follow it. Keep framework-specific examples where they teach an actual integration. Omit personal repository arrangements, the unpublished predecessor's migration history, and maintainer-only release or publishing procedures. State verified APIs, compatibility, and limitations; do not imply adoption, maturity, or capabilities that have not been established.
...
- **`<Problem id="..." />`** — at the end of doc topic pages under `## 例题`. It loads the matching `docs/contest/_problems/<id>/index.mdx` partial; these files never create standalone routes. A problem partial has no YAML front matter and instead begins with one colocated MDX export:

  ```js
  export const problem = {
    title: '洛谷 P1001 A+B Problem',
    link: 'https://www.luogu.com.cn/problem/P1001',
  };
  ```

  Keep only `title` and `link` in this object, then write the statement body directly below it. Code files remain beside the partial in the same problem directory.
...
- Markdown text links receive their destination icon automatically. Do not insert a second icon into the link label; fragment links and image-only links keep their original appearance.
...
AI 洛谷题解只在 `docs/contest/personal/luogu.mdx` 的 `### AI 题解` 下建立链接索引。该板块
紧接题解示例，所有条目共用一个无序列表；不论数量多少都不分页、不拆分，也不建立
`ai-solutions-*.mdx` 博客或逐题页面。每项使用完整题解标题和洛谷专栏链接，不复制正文。
维护时核对板块位置、完整标题、链接、重复题号与重复专栏，并确认没有恢复合集博客。
