---
title: 特征值与相似矩阵
description: 本文讲解特征值与特征向量，含特征方程、相似与可对角化条件、内积、施密特正交化与实对称矩阵正交对角化。
summary: 线性变换中方向不变、只被伸缩的向量即特征向量，伸缩倍数即特征值。由特征方程求特征值、再解齐次方程得特征子空间，并用迹等于特征值之和、行列式等于其积自检。梳理相似矩阵及其不变量、代数重数与几何重数，给出可对角化的充要条件、步骤与求矩阵高次幂的应用。后半转入向量内积、施密特正交化与正交矩阵，落到实对称矩阵：特征值全为实数、必能用正交矩阵对角化，为二次型化标准形铺路。
---
...
\end{pmatrix},Q^TAQ=\begin{pmatrix}
  5 & & \\
  & -1 & \\
  & & -1
\end{pmatrix}
