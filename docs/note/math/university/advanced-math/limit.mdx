---
title: 函数与极限
description: 本文将介绍作为微积分地基的极限，含数列与函数极限、无穷小与等价无穷小、未定式，以及连续与间断点。
summary: 极限作为微积分地基的完整梳理。数列极限从 ε-N 定义讲到单调有界、夹逼、柯西准则与 Stolz 定理，函数极限给 ε-δ 定义、单侧极限与 Heine 归结原则。随后是极限四则运算、无穷小与无穷大及其阶、等价无穷小表与无穷大增长排序，七种未定式的化归思路与两个重要极限。计算方法逐一演示代入、等价无穷小、泰勒展开、洛必达与夹逼。最后讲连续定义、间断点分类与闭区间连续函数的介值、零点定理。
---
...
$$
\lim_{n\to\infty}\frac{a_n}{b_n}=\lim_{n\to\infty}\frac{a_n-a_{n-1}}{b_n-b_{n-1}}(\text{右端存在时})
$$
...
$$
\begin{aligned}
  \text{有界：} &\text{归纳证 }a_n<2.a_1<2;\text{if }a_n<2\text{ then }a_{n+1}=\sqrt{2+a_n}<\sqrt{4}=2. \\
...
$$
\lim_{x\to x_0^-}f(x)=A^-,\lim_{x\to x_0^+}f(x)=A^+
$$
...
$$
1-\cos x\sim\frac{x^2}{2},e^x-1\sim x,a^x-1\sim x\ln a,\ln(1+x)\sim x,(1+x)^a-1\sim ax
$$
...
$$
v(u-1)=x\cdot\frac{(x+1)-(x-1)}{x-1}=\frac{2x}{x-1}\xrightarrow{x\to\infty}2,\text{原式}=e^2
$$
...
例：$\lim\limits_{x\to 0}\dfrac{x-\sin x}{x^3}\xlongequal{\frac{0}{0}}\lim\limits_{x\to 0}\dfrac{1-\cos x}{3x^2}\xlongequal{\frac{0}{0}}\lim\limits_{x\to 0}\dfrac{\sin x}{6x}=\dfrac{1}{6}$。连用两次，每次都验型是 $\dfrac00$。
...
通项被两个同极限的式子夹住时用夹逼，**带 $\sin\frac{1}{x}$、含取整、$n$ 项求和** 的极限尤其常见。
