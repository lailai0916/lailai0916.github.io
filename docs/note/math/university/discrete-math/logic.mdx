...
**换名规则**：约束变元可以改名（$\forall xP(x)\Leftrightarrow\forall yP(y)$），但新名字不能与辖域内已有的其他变元冲突，否则会改变含义。这跟编程里函数局部变量改名同理——只要不撞名，叫什么都行。
...
$$
\forall x(P(x)\land Q)\Leftrightarrow\forall xP(x)\land Q,
\exists x(P(x)\lor Q)\Leftrightarrow\exists xP(x)\lor Q
$$
...
$$
\begin{aligned}
  \forall xP(x)\to\exists xQ(x) & \Leftrightarrow\neg\forall xP(x)\lor\exists xQ(x) & & \text{蕴含等价} \\
