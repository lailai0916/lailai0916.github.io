...
最后一列恒为 $1$，故它是 **重言式**——这正是逆否等价 $p\to q\iff\neg q\to\neg p$ 的另一种说法。
...
若 $A\leftrightarrow B$ 是重言式，称 $A,B$ **等值**，记 $A\iff B$。等值式是公式化简的「运算法则」，相当于代数里的恒等式。

|              |                                                |
| :----------- | :--------------------------------------------- |
| **双重否定** | $\neg\neg p\iff p$                             |
| **幂等律**   | $p\land p\iff p$，$p\lor p\iff p$              |
| **交换律**   | $p\land q\iff q\land p$，$p\lor q\iff q\lor p$ |
| **结合律**   | $(p\land q)\land r\iff p\land(q\land r)$       |
| **分配律**   | $p\land(q\lor r)\iff(p\land q)\lor(p\land r)$  |
| **德摩根律** | $\neg(p\land q)\iff\neg p\lor\neg q$           |
| **吸收律**   | $p\lor(p\land q)\iff p$                        |
| **零一律**   | $p\lor 1\iff 1$，$p\land 0\iff 0$              |
| **同一律**   | $p\lor 0\iff p$，$p\land 1\iff p$              |
| **排中律**   | $p\lor\neg p\iff 1$                            |
| **矛盾律**   | $p\land\neg p\iff 0$                           |
| **蕴含等价** | $p\to q\iff\neg p\lor q$                       |
| **逆否等价** | $p\to q\iff\neg q\to\neg p$                    |
| **等价展开** | $p\leftrightarrow q\iff(p\to q)\land(q\to p)$  |
...
例：证明 $(p\to q)\land(p\to r)\iff p\to(q\land r)$。

$$
\begin{aligned}
  (p\to q)\land(p\to r) & \iff(\neg p\lor q)\land(\neg p\lor r) & & \text{蕴含等价} \\
  & \iff\neg p\lor(q\land r) & & \text{分配律} \\
  & \iff p\to(q\land r) & & \text{蕴含等价}
\end{aligned}
$$
...
$$
A_1\land A_2\land\dots\land A_n\implies B
$$
...
| 名称       | 规则                                                  |
| :--------- | :---------------------------------------------------- |
| 假言推理   | $(p\to q)\land p\implies q$                           |
| 拒取式     | $(p\to q)\land\neg q\implies\neg p$                   |
| 析取三段论 | $(p\lor q)\land\neg p\implies q$                      |
| 假言三段论 | $(p\to q)\land(q\to r)\implies p\to r$                |
| 化简律     | $p\land q\implies p$                                  |
| 附加律     | $p\implies p\lor q$                                   |
| 构造性二难 | $(p\to q)\land(r\to s)\land(p\lor r)\implies q\lor s$ |
...
**换名规则**：约束变元可以改名（$\forall xP(x)\iff\forall yP(y)$），但新名字不能与辖域内已有的其他变元冲突，否则会改变含义。这跟编程里函数局部变量改名同理——只要不撞名，叫什么都行。
...
$$
\forall x(P(x)\land Q)\iff\forall xP(x)\land Q,
\exists x(P(x)\lor Q)\iff\exists xP(x)\lor Q
$$

**量词分配**：

$$
\forall x(P(x)\land Q(x))\iff\forall xP(x)\land\forall xQ(x)
$$

$$
\exists x(P(x)\lor Q(x))\iff\exists xP(x)\lor\exists xQ(x)
$$
...
$$
\exists y\forall xP(x,y)\implies\forall x\exists yP(x,y)
$$

只能这样单向推，反向不成立。设 $P(x,y)$ 为「$y$ 是 $x$ 的母亲」：

- $\forall x\exists yP(x,y)$：每个人都有母亲（母亲可以因人而异）——真。
- $\exists y\forall xP(x,y)$：存在一个人是 **所有人** 共同的母亲——假。
...
例：求 $\forall xP(x)\to\exists xQ(x)$ 的前束范式。两个量词都用了 $x$，直接提取会冲突，所以先换名再提取：

$$
\begin{aligned}
  \forall xP(x)\to\exists xQ(x) & \iff\neg\forall xP(x)\lor\exists xQ(x) & & \text{蕴含等价} \\
  & \iff\exists x\neg P(x)\lor\exists xQ(x) & & \text{量词否定} \\
  & \iff\exists x\neg P(x)\lor\exists yQ(y) & & \text{换名} \\
