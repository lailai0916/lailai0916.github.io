---
title: 三角函数
description: 本文将介绍任意角与弧度制、单位圆定义下的三角函数性质与恒等式，并从余弦差角公式统一推出各类三角公式。
summary: 从任意角与弧度制、单位圆讲起，先用直角三角形定义锐角三角函数，再用单位圆把它推广到任意角，得出单位圆上辐角 θ 的点横纵坐标恰为余弦与正弦。性质部分列六个三角函数的定义域、值域、奇偶性、周期与常用值速查表。恒等式部分讲平方、商数、倒数、积的关系。公式部分以余弦差角公式为唯一几何推导起点，依次导出两角和差、诱导、二倍角、三倍角、半角、积化和差、和差化积、辅助角与万能公式。末尾扩展反三角、双曲函数、简谐运动与傅里叶变换。
---
...
锐角三角函数定义是基于 **直角三角形** 的，但直角三角形的锐角只能在 $\left(0,\frac{\pi}{2}\right)$ 范围内。
...
|  值域  |   $y\in[-1,1]$   |   $y\in[-1,1]$   |     $y\in\mathbb{R}$     | $y\in\mathbb{R}$ |    $y\le-1\lor y\ge1$    | $y\le-1\lor y\ge1$ |
...
$$
\sin\left(\frac{\pi}{2}-\theta\right)=\cos\left(\frac{\pi}{2}-\left(\frac{\pi}{2}-\theta\right)\right)=\cos\theta
$$
...
$$
\sin\left(\frac{\pi}{2}+\alpha\right)=\sin\frac{\pi}{2}\cos\alpha+\cos\frac{\pi}{2}\sin\alpha=\cos\alpha
$$
...
| :------: | :--------------------------------------------: | :----------: | :----------: | :----------: | :----------: | :----------: | :----------: |
| 第一象限 |    $\left(2k\pi,2k\pi+\frac{\pi}{2}\right)$    |     $+$      |     $+$      |     $+$      |     $+$      |     $+$      |     $+$      |
| 第二象限 |  $\left(2k\pi+\frac{\pi}{2},2k\pi+\pi\right)$  |     $+$      |     $-$      |     $-$      |     $-$      |     $-$      |     $+$      |
| 第三象限 | $\left(2k\pi+\pi,2k\pi+\frac{3\pi}{2}\right)$  |     $-$      |     $-$      |     $+$      |     $+$      |     $-$      |     $-$      |
| 第四象限 | $\left(2k\pi+\frac{3\pi}{2},2k\pi+2\pi\right)$ |     $-$      |     $+$      |     $-$      |     $-$      |     $+$      |     $-$      |

原理：单位圆具有对称性和周期性；所有三角函数在第一象限内均为正数。

:::example

化简 $\sin\left(\frac{3\pi}{2}-\alpha\right)$。

1. $\frac{3\pi}{2}=3\cdot\frac{\pi}{2}$，系数 $3$ 是奇数，所以要将 $\sin$ 变为 $\cos$。
2. 把 $\alpha$ 看作第一象限角，则 $\frac{3\pi}{2}-\alpha$ 为第三象限角，$\cos$ 在第三象限为负数，所以为负号。

综上所述，$\sin\left(\frac{3\pi}{2}-\alpha\right)=-\cos\alpha$。
...
\begin{aligned}
  ...
  & =\frac{4\sin\alpha\sin\left(\frac{\pi}{3}+\alpha\right)\sin\left(\frac{\pi}{3}-\alpha\right)}{4\cos\alpha\cos\left(\frac{\pi}{3}-\alpha\right)\cos\left(\frac{\pi}{3}+\alpha\right)} \\
  & =\tan\alpha\tan\left(\frac{\pi}{3}-\alpha\right)\tan\left(\frac{\pi}{3}+\alpha\right)
\end{aligned}
$$
...
\begin{array}{l}
  ...
  \implies\cos^2\frac{\alpha}{2}=\frac{1+\cos\alpha}{2} \\
  \implies\cos\frac{\alpha}{2}=\pm\sqrt{\frac{1+\cos\alpha}{2}}
\end{array}
$$
...
\begin{array}{l}
  ...
  \implies\sin^2\frac{\alpha}{2}=\frac{1-\cos\alpha}{2} \\
  \implies\sin\frac{\alpha}{2}=\pm\sqrt{\frac{1-\cos\alpha}{2}}
\end{array}
$$
...
详见 [直线和圆](/docs/note/math/geometry/line-and-circle)。
