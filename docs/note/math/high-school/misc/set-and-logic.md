...
在国内数学教育体系中，**集合** 是继 **数** 之后，第一个具备运算和关系的抽象数学结构。
...
任意一个元素要么属于这个集合，要么不属于这个集合。

不允许出现“高个子的人”这种模棱两可的情况。
...
元素通常用 $a,b,\cdots$ 等小写字母表示，而集合通常用 $A,B,\cdots$ 等大写字母表示。
...
2. **描述法**：列举集合中 **元素满足的条件**，用竖线（$|$）、冒号（$:$）或分号（$;$）分隔。
...
我们将包含集合 $A$ 和 $B$ 中所有元素的集合 $C$ 定义为 **并集**，记作：
