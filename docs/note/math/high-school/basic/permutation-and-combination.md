...
加法原理：完成一项任务有 $n$ 类方式，第 $i$ 类有 $a_i$ 种方法。则完成该任务的总方法数为：
...
1. 乘地铁：可以选择地铁 $1$ 号线或 $5$ 号线，共 $2$ 种；
2.	乘公交车：可以选择公交 $26$ 路、$38$ 路或 $45$ 路，共 $3$ 种。

根据加法原理，共有 $2+3=5$ 种不同的出行方式。

:::

### 乘法原理

乘法原理：完成一项任务有 $n$ 个步骤，第 $i$ 个步骤有 $a_i$ 种方法。则完成该任务的总方法数为：

$$
S=\prod_{i=1}^n a_i=a_1\times a_2\times\dots\times a_n
$$
...
根据乘法原理，共有 $4\times 3=12$ 种不同的穿衣搭配。
...
全排列：把 $n$ 个物品排成一列，有 $n!$ 种不同的排列。

### 排列

排列：从 $n$ 个不同元素中取 $m$ 个排成一列，**考虑顺序**，产生不同排列的数量为：
