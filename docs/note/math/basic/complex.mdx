---
title: 复数
description: 本文将介绍复数的定义、复平面几何表示、四则运算与幂，以及三角表示、欧拉公式与四元数。
summary: 从无理数与虚数的历史危机切入，定义虚数单位 i 与复数 z=a+bi 的实部、虚部，给出复数集 C 与实部虚部相等的判定。几何部分把复数对应到复平面上的点或向量，讲模长、辐角、共轭复数与分母实数化。运算部分讲加减乘除与幂：加减遵循平行四边形法则，乘除对应模长相乘相除、辐角相加相减。三角表示部分由直角坐标与极坐标的转换引出三角形式，并补充 atan2。末尾扩展到欧拉公式与四元数。
---
...
类似的故事在代数学中再一次上演。当人们研究二次方程 $x^2+1=0$ 时，发现它在实数范围内无解，从而引出了虚数单位 $i=\sqrt{-1}$ 的概念。虽然起初这种「负数开方」显得荒诞，但在更复杂的三次方程（$ax^3+bx^2+cx+d=0,a\ne0$）中，它却成为必不可少的工具。
...
数集在前面的 [集合](/docs/note/math/basic/set#数集) 章节中提到过，我们在实数集 $\mathbb{R}$ 的基础上扩展出 **复数集** $\mathbb{C}$。
...
复数的扩展是合理的，因为这个体系自洽，即内部没有矛盾——在现代数学中，逻辑自洽即合理。
...
我们将 [向量](/docs/note/math/basic/vector) $\overrightarrow{OZ}$ 的 **模长** 定义为复数 $z=a+bi$ 的 **模长**，记为：
...
我们将向量 $\overrightarrow{OZ}$ 与 **实轴正方向** 的夹角定义为 **辐角**，记作 $\arg z$，取值范围通常为 $[0,2\pi)$。

这类似于任意角 [三角函数](/docs/note/math/function/trigonometric) 的定义。
...
在初中阶段，我们学习了 **直角坐标系**（笛卡尔坐标系），这是最常用的坐标系。
...
从 **极坐标系** 转换到 **直角坐标系** 可以使用 [三角函数](/docs/note/math/function/trigonometric)：
