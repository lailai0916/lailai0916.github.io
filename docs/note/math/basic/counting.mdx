---
title: 计数
description: 本文将介绍两大计数原理、排列组合与组合数性质、插板法分配、二项式定理与多项式展开，以及杨辉三角。
summary: 从加法原理与乘法原理出发，推导全排列、阶乘、排列数 A 与组合数 C，列出组合数的对称、递推与求和等性质。应用部分用插板法处理正整数解、非负整数解与带下界的分配问题，以及相同元素、不同元素分组的计数。随后由展开式中选取因子的组合意义得出二项式定理，并给出三项式的多项式展开公式与若干配凑系数的例子。末尾把二项式系数排成杨辉三角，说明每个数等于左上与右上之和，恰对应一个组合数。
---
...
本质是求 $x_1+x_2+\dots+x_k=n$ 满足 $x_i\ge a_i$ 的整数解的数目。
...
$$
(x_1'+a_1)+(x_2'+a_2)+\dots+(x_k'+a_k)=n\implies\sum_{i=1}^kx_i'=n-\sum a_i
$$
...
$$
\mathrm{C}_n^{n_1}\mathrm{C}_{n-n_1}^{n_2}\dots\mathrm{C}_{n_a}^{n_a}=\frac{n!}{n_1!\times n_2!\times\dots\times n_a!}=\frac{n!}{\prod_{i=1}^a(n_i!)}
$$
