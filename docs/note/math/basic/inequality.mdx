---
title: 不等式
description: 本文将介绍不等式的基本性质、一元二次不等式求解与基本不等式，并扩展到四种均值关系与柯西等经典不等式。
summary: 从等式与不等式的基本性质出发，讲一元二次不等式的求解：化标准形式、算判别式、按各种情况对照解集。基本不等式一节由完全平方非负推出两正数的算术平均不小于几何平均，给出代数与几何两种证法。扩展部分把两数均值推广到调和、几何、算术、平方四个平均数的大小关系，并收录糖水不等式、柯西-施瓦茨不等式、权方和不等式，每条配一道求最值的例题。
---
...
- $a=a$（自反性）
- $a=b\implies b=a$（对称性）
- $a=b,b=c\implies a=c$（传递性）
- $a=b\implies a\pm c=b\pm c,ac=bc,\frac{a}{c}=\frac{b}{c}$（$c\ne 0$）（替代性）
...
- $a>b,c>0\implies ac>bc,c<0\implies ac<bc$
- $a>b,c>d\implies a+c>b+d$（加法单调性）
- $a>b>0,c>d>0\implies ac>bd$（乘法单调性）
- $a>b>0,n>0\implies a^n>b^n,n<0\implies a^n<b^n$
...
$$
ax^2+bx+c<0\text{ or }ax^2+bx+c>0(a\ne 0)
$$
...
利用 [完全平方差公式](/docs/note/math/basic/transition#平方公式) 可以得到一个不等式：
...
因为 $AB\ge DE$，所以 $a+b\ge 2\sqrt{ab}$。当且仅当 $a=b$ 时，等号成立。
...
$a$ 克糖水中有 $b$ 克糖（$a>b>0$），则质量分数（即糖和糖水的质量比）为：

$$
w_1=\frac{b}{a}
$$

若再添加 $c$ 克糖（$c>0$），则质量分数为：
...
$$
(5x+2y)^2\le 87\implies 5x+2y\le\sqrt{87}
$$
...
$$
1\ge\frac{9}{x+y}\implies x+y\ge 9
$$
