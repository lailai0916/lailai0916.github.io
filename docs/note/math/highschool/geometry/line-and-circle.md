...
构造直角三角形 $\triangle ABC$，两条直角边的长度分别为：

$$
|AC|=|x_0-x_1|,|BC|=|y_0-y_1|
$$

根据勾股定理，斜边的长度 $|AB|$ 为：

$$
|AB|=\sqrt{|AC|^2+|BC|^2}=\sqrt{(x_0-x_1)^2+(y_0-y_1)^2}
$$
...
常见的直线方程有 **斜截式**、**两点式**、**点斜式**、**截距式**、**一般式**、**点向式** 和 **参数式** 等。

### 点斜式

确定一条直线需要两条信息，假设我们知道了一条直线过一个点 $(x_0,y_0)$，那么我们只需要知道这条直线的倾斜角或者斜率，就可以确定这条直线了。

我们定义直线的方向为直线向上的方向，这个方向与 $x$ 轴正半轴的夹角 $\theta$，记为这条直线的 **倾斜角**，倾斜角的范围是 $\theta\in[0,\pi)$。特殊地，规定 $x$ 轴的倾斜角为 $0$。
...
也就是，$x$ 每增加 $1$，$y$ 的变化量就是直线的斜率，特殊地，我们不定义平行于 $y$ 轴的直线的斜率。容易得到：
...
特殊地，如果直线过原点，则 $C=0$；对于斜率不存在的，直线方程就为 $x=x_0$。

写一般式技巧：

- 用两种方式表示斜率，例如用点 $(x,y)$ 和任意给定的一点，给定的两点，然后化简。

## 圆的方程

在有关圆的问题中几何法往往是第一选择，但解析几何的意思就是用代数的方法研究几何问题。在这里代数方法指的就是坐标法求出坐标成了最关键的一步，而求坐标的大前提就是需要联立直线与圆的方程，我们会在圆锥曲线中详细讨论这个问题。
...
因此只要 $D^2+E^2>4F$，该方程就可以表示一个圆了；特殊地，如果 $D=E=F$ 则该方程表示一个点 $(0,0)$。具体地，圆心：$\left(-\frac{D}{2},-\frac{E}{2}\right)$，半径：$\frac{\sqrt{D^2+E^2-4F}}{2}$。
...
- 一般方程判断点是否在圆内、外，只需要用左式与 $0$ 比较即可，大于 $0$ 在圆外，小于 $0$ 在圆内。
...
- 除非特殊优势，通常情况下可以常用点斜式和一般式来进行书写直线方程。
