...
- [圆锥曲线的离心率到底离了什么心？【MaExpo获奖作品】 - bilibili](https://www.bilibili.com/video/BV1CvPyeaEMv)
...
我们不妨代入这五个点（公式略），这是一个有 $5$ 个方程、$6$ 个未知数的齐次线性方程组。根据线性代数理论，这样的方程组总是有非零解。如果解空间的维数是 $1$，那么所有的非零解都是成比例的，它们对应同一条圆锥曲线。这就意味着，这五个点唯一确定了一条圆锥曲线。
...
这五个点的位置关系直接决定了最终曲线的类型。例如，如果五个点构成一个凸五边形，那么它们确定的曲线必然是椭圆。如果一个点远离其他四个点，则很可能形成双曲线或抛物线。
...
阿波罗尼斯圆（阿氏圆）已知平面上两点 $A,B$，则所有满足 $PA/PB=k$（$k\neq1$）的点 $P$ 的轨迹是一个圆。
...
我们不妨设 $F_1=(-c,0),F_2=(c,0)$ 为焦点的椭圆经过点 $M$，且 $M$ 到 $F_1,F_2$ 的距离之和为 $2a$，其中 $a>c$，那么写出方程：
...
因为我们取的是右焦点，因此这个准线也称为右准线；同理取左焦点
...
也就是说，椭圆整个图形都在一个中心在原点、长为 $2a$ 高为 $2b$ 的矩形中，矩形与椭圆切点分为上下左右四个。
...
与椭圆类似，我们设左焦点 $F_1=(-c,0)$ 和右焦点 $F_2=(c,0)$，距离之差为 $2a$，其中 $c>a>0$，有双曲线的标准方程
...
我们知道三角函数 $\sin\theta,\cos\theta$ 是在单位圆上引入一条倾角为 $\theta$ 的直线，与单位圆相交的点的坐标即为 $(\cos\theta,\sin\theta)$，类似地我们推出双曲函数的最初定义：倾角为 $\theta$ 的直线与双曲线 $x^2-y^2=1$ 交于点 $(\cosh\theta,\sinh\theta)$。
...
4. 我们做推广，两条切线为 $\varphi$ 或 $180^\circ-\varphi$ 的点的集合是一个以 $F$ 焦点，抛物线准线 $\ell$ 为准线的双曲线，其离心率等于 $\frac{1}{|\cos\varphi|}$。
...
设光线从 $F_1$ 经过直线 $\ell$ 的反射到达点 $F_2$，我们将 $F_1$ 关于 $\ell$ 对称的点设为 $F_1'$

![](https://cloud.lailai.one/f/YXpHA/conic-image-10.png)

则点 $P$ 位于 $F_1'F_2$ 与 $\ell$ 的交点上，这就是将军饮马模型，也容易证明，入射角和出射角是相等的。
...
我们不妨证明过点 $P$ 的切线 $\ell$ 恰好平分 $\angle FPP'$。
...
对双曲线而言，这样的圆并不总存在。当双曲线的两条渐近线所成角为锐角时，该圆的半径为虚数；若两条渐近线相互垂直，则该圆退化为双曲线的中心这一点。
...
\end{bmatrix}=\left[{\begin{array}{ccc|c}
  \, & A &  & {\vec{b}}\ \\
  0 & \dots & 0 & 1
\end{array}}\right]{\begin{bmatrix}{\vec {x}}
   \\
  1
\end{bmatrix}}
$$
