---
title: 解三角形
description: 本文将介绍解三角形的正弦余弦定理与边角互化、三角形四心与三线定理、面积公式，并延伸到四边形面积公式。
summary: 解三角形的方法与定理汇编。约定边角记号后，围绕正弦定理、余弦定理讲边角互化与选择原则，补上射影定理、正切定理、内切圆与外接圆半径。三角形四心连同奔驰定理、欧拉线、极化恒等式统一用向量刻画，三线部分给出中线长、分角、角平分线定理，边长部分含斯图尔特与梅涅劳斯定理。面积公式列全底高、两边夹角、海伦、秦九韶等写法，并延伸到布雷特施奈德与婆罗摩笈多四边形公式。
---
...
$$
\sin 15^\circ=\cos 75^\circ={\sqrt6-\sqrt2\over4}
$$

$$
\sin 75^\circ=\cos 15^\circ={\sqrt6+\sqrt2\over4}
$$
...
- 设 $O$ 为外心，则 $\sin 2A\cdot\overrightarrow{OA}+\sin 2B\cdot\overrightarrow{OB}+\sin 2C\cdot\overrightarrow{OC}=\vec0$.
...
$$
\vec{PG}=\frac{1}{3}\left(\vec{PA}+\vec{PB}+\vec{PC}\right)
$$
...
$$
\vec{HA}\cdot\vec{BC}=\vec{HB}\cdot\vec{AC}=\vec{HC}\cdot\vec{AB}=0
$$
...
$$
S=\frac{1}{2}ab\sin{\angle C}=\frac{1}{2}ac\sin{\angle B}=\frac{1}{2}bc\sin{\angle A}
$$
...
$$
S=\frac{abc}{4R}=2R^2\sin{\angle A}\sin{\angle B}\sin{\angle C}=\frac{1}{2}R^2(\sin{2\angle A}+\sin{2\angle B}+\sin{2\angle C})
$$
...
注意到圆内接四边形对角互补，其半角余弦值为零，$\cos 90^\circ=0$ 则圆内接四边形面积可以简化为：
