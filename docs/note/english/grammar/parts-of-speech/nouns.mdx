---
title: 名词
description: 本文讲解英语名词的分类、可数与不可数、复数构成及名词所有格
summary: 英语名词的系统笔记。名词按意义分普通名词与专有名词。普通名词又分可数与不可数：可数名词有单复数，梳理规则复数的构成与不规则变化，并区分个体名词与集体名词；不可数名词含物质名词与抽象名词，说明其借助计量单位表数量及抽象名词具体化后可数的情形。专有名词首字母大写、一般不可数，特定语境下可变可数。最后讲名词所有格的 's 所有格、of 所有格与双重所有格三种形式及其语义差别。
---

## 简介

**名词**（Noun）是表示 **人**、**事物**、**地点**、**机构** 或 **抽象概念** 名称的词。

名词在句中可作 **主语**、**宾语**、**表语**、**定语**、**同位语**、**呼语** 等成分。

按 **意义** 可分为 **普通名词** 和 **专有名词**：

- **普通名词**：泛指某类人、事物的通用名称。
- **专有名词**：特指特定的人、地、机构等，**首字母大写**。

## 普通名词

普通名词（Common Noun）是表示某类人、事物、地点或抽象概念的通用名称。

### 名词的数

普通名词分为 **可数名词** 和 **不可数名词**。

可数名词有 **单数** 和 **复数** 两种形式。

#### 构成规则

1. 一般情况：加 -s。
2. 特殊规则（为保持发音或拼写规律）。
   1. 以 -s, -x, -sh, -ch 结尾：加 -es。
   2. 以「辅音+y」结尾：将 y 改为 i，再加 -es。
   3. 以 f 或 fe 结尾：将 f 改为 v，再加 -es。
   4. 以 o 结尾：加 -s 或 -es。
3. 不规则变化。

:::tip

以 o 结尾的名词的复数形式：英语本身的普通名词（前为辅音字母），通常加 -es；外来词、缩写、科技词或音节轻的词，通常加 -s。

:::

:::example

- carrot（胡萝卜）$\to$ carrots
- rabbit（兔子）$\to$ rabbits

:::

:::example

- box（盒子）$\to$ boxes
- wish（愿望）$\to$ wishes
- bus（公交车）$\to$ buses
- watch（手表）$\to$ watches

:::

:::example

- city（城市）$\to$ cities
- century（世纪）$\to$ centuries
- baby（婴儿）$\to$ babies
- country（国家）$\to$ countries

:::

:::example

- knife（刀）$\to$ knives
- wife（妻子）$\to$ wives
- leaf（叶子）$\to$ leaves

:::

:::example

- tomato（番茄）$\to$ tomatoes
- potato（土豆）$\to$ potatoes
- hero（英雄）$\to$ heroes
- mango（芒果）$\to$ mangoes

:::

:::example

- man（男人）$\to$ men
- foot（脚）$\to$ feet
- mouse（老鼠）$\to$ mice
- child（孩子）$\to$ children

:::

### 可数名词

可数名词（Countable Noun）是指能够用数词计算的具体事物。

#### 个体名词

表示个体的人或事物的词。

:::example

- boy（男孩）、carrot（胡萝卜）、rabbit（兔子）

:::

:::tip

名词本身可体现单复数信息，这是屈折语的一大优势。

:::

#### 集体名词

表示一群人或一类事物的词，根据语境可视为单数或复数。

:::example

- family（家庭）、team（团队）、audience（观众）、fruit（水果）

:::

- **单数用法**：The team is large.
- **复数用法**：The team are working together.
- 有的集合名词只作复数：police、people
- 有的集合名词只作单数：furniture、baggage

### 不可数名词

不可数名词（Uncountable Noun）是无法用数词直接计算的名词，通常无复数形式。

#### 物质名词

表示无法分为个体的东西的词。

:::example

- water（水）、milk（牛奶）、bread（面包）、air（空气）、beer（啤酒）、wood（木头）、paper（纸）

:::

- 通常不说 `two waters`，而说 `two glasses of water`
- 若 water 表示「水域」时可数，例如：**international waters**

#### 抽象名词

表示状态、品质、行为、感情等抽象概念等词。

:::example

- power（力量）、peace（和平）、honesty（诚实）、pleasure（愉悦）

:::

:::tip

有时抽象名词也可数。例如：

- Thank you → It's a pleasure.

此时 pleasure 表示「一件愉快的事」，即具体化。

:::

## 专有名词

表示人、地方、事物、机构、组织等的词，通常 **首字母大写**，一般 **不可数**。

:::example

- 人名：Jack、Harry Potter、Michael
- 地名：Earth（地球）、Asia（亚洲）、China（中国）、London（伦敦）
- 机构名：United Nations（联合国）、Bank of China（中国银行）
- 时间名：Monday（星期一）、August（八月）、New Year（新年）

:::

:::tip

专有名词在某些语境下可以变为可数：

- There's a Michael downstairs.（楼下有个叫 Michael 的人。）（泛指众多 Michael 中的一个）
- There are three Michaels in my class.（我们班有三个叫 Michael 的人。）

:::

## 名词所有格

### 's 所有格

用于表示所属关系，常用于有生命的事物，也可用于无生命名词表达性质。

:::example

- The rabbit's carrot（兔子的胡萝卜）
- Michael's computer（Michael 的电脑）
- Tom and Jerry's room（Tom 和 Jerry 共有的房间）
- Today's news（今天的新闻）

:::

:::tip

- 并列名词共有物：在最后一个名词后加 `'s`
- 各自拥有物：各个名词分别加 `'s`，名词变复数

:::

### of 所有格

用于表示关系或特征，结构为：`A of B`，其中 B 修饰 A。

:::example

- The brightness of the moon（月亮的亮度）
- A friend of Michael（Michael 的一个朋友）
- A photo of Michael（Michael 出现在里面的照片）

:::

:::tip

- `A photo of Michael's` → Michael 拍的照片
- `A photo of Michael` → Michael 出现在照片中
- `A friend of Michael's` = one of Michael's friends

:::

### 双重所有格

双重所有格主要表示整体中的一部分，有两种形式：

1. 名词 + of + 's 所有格

:::example

- I am a friend of Michael's.（我是 Michael 的一个朋友。）
- That is a photo of Alice's.（那是 Alice 拍的一张照片。）

:::

2. 名词 + of + 名词性物主代词

:::example

- I lost a book of mine.（我丢了一本我自己的书。）

:::

## 思维导图

```mermaid
graph TD;
    A1[名词];
    A1 --> B1[普通名词];
    A1 --> B2[专有名词];
    A1 --> B3[名词所有格];
    B1 --> C1[可数名词];
    B1 --> C2[名词的数];
    B1 --> C3[不可数名词];
    B3 --> C4['s 所有格];
    B3 --> C5[of 所有格];
    B3 --> C6[双重所有格];
    C1 --> D1[个体名词];
    C1 --> D2[集体名词];
    C3 --> D3[物质名词];
    C3 --> D4[抽象名词];
```
