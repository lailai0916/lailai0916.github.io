...
圆周的长度与半径之比为 $2\pi$，因此 $360^\circ = 2\pi$ 弧度。
...
距离公式：$A(x_0,y_0),B(x_1,y_1)$ 两点之间的距离 $d=\sqrt{(x_0-x_1)^2+(y_0-y_1)^2}$。

<Desmos url="https://www.desmos.com/calculator/0nqvqsggg1?embed" />

推导：通过构造直角三角形，不难发现两条直角边分别为 $(x_1-x_0)$ 和 $(y_1-y_0)$，而斜边就是 $A$ 点和 $B$ 点的距离 $d$，根据勾股定理：

$d^2=(x_0-x_1)^2+(y_0-y_1)^2$

所以 $d=\sqrt{(x_0-x_1)^2+(y_0-y_1)^2}$.
...
不要搞反，这个点坐标是 $(\cos{\theta},\sin{\theta})$，$\cos$ 在前面.
...
这是唯一需要几何推导证明的公式，其他公式都是用另外的公式推出来的.
