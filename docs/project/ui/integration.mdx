...
- [Home 接入实现 - GitHub](https://github.com/lailai0916/lailai0916.github.io/tree/main/src/components/LaikitProvider)
...
在主题根节点包裹页面。Home 已有 `src/components/LaikitProvider`，页面直接使用组件即可。该适配还通过 Docusaurus 的翻译函数提供库内文案，保留站点既有的翻译管理方式。

其他路由框架同样实现 `LinkProps` 适配：站内路径交给路由组件，外部地址保留正常链接行为。不要把框架依赖加入共享组件库。
