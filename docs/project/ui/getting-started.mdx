---
title: 快速开始
description: 本文介绍共享组件库的安装方式、样式导入顺序，以及可直接使用的 React 应用示例。
...
`LaikitProvider` 负责路由、标题与内置文案；`ThemeProvider` 负责整份文档的主题。Docusaurus 已有主题管理器，接入时只使用前者，详见[框架接入](/docs/project/ui/integration)。

## 效果展示

<Card>
  <Stack gap={12}>
    <span>同一套组件随网站的主题与强调色变化。</span>
    <Cluster>
