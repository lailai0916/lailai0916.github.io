---
title: lailai's Home
description: 本文介绍个人网站的选型与搭建历程，提供维护入口，以及安装、预览和部署教程。
summary: lailai 个人网站采用 Docusaurus，使用树状文档整理算法竞赛知识、学习笔记与项目，也保留博客内容。项目页回顾平台与域名的选择，提供仓库和 Agent 维护入口，并保留基于 Docusaurus v3.8 的历史搭建教程，演示 TypeScript 项目的创建、启动、预览与 GitHub Pages 部署。网站内部规则负责内容格式和工程约束，个人风格通过外部 lailai-skill 引用。末尾列出相关插件与个人网站示例。
---
...
域名我也纠结了很久。其实我在 2021 年 5 月 25 日就注册了 `lailai0916.com`，但总觉得有点长，一直想要 `lailai.com`。可惜这个域名是美国「莱莱食府」餐厅的官网，我想花几百美元买下来，但中介说对方已经拒绝过 1 万美元了。后来我又考虑过 `lailai.one`、`lailai.top`、`lailai.cc`、`lailai.io`、`lailai.ai`、`lailai.me` 等，最后在 2024 年 10 月 16 日选定并注册了 `lailai.one`。

2024 年 10 月，我开始动手搭建网站，但 Docusaurus 在国内互联网比较冷门，相关的资料和教程并不多，而且在此之前我没有系统学习过 HTML、CSS 和 JavaScript 语言，凭借 C++ 和 Python 等语言的基础和 AI 的帮助，在建站的过程中逐渐掌握了一些前端基本语法。
...
维护本站从仓库的 [AGENTS.md](https://github.com/lailai0916/lailai0916.github.io/blob/main/AGENTS.md) 开始，再读取待修改路径对应的内部规则。

本站维护 frontmatter、MDX 组件、内容分类、多语言和工程约束。跨项目的个人表达与代码风格由 [lailai.skill](/docs/project/personal/lailai-skill) 提供，专用任务也通过它选择对应流程；网站不嵌入 Skill 副本或子模块。
...
- [Mike Barton](https://www.mikes.cv)
