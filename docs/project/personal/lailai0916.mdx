---
title: lailai0916
description: 本文介绍搭建 GitHub 主页 README 教程，演示建同名仓库、多语言标题、嵌入图标与统计图。
summary: 搭建 GitHub 个人主页 README 的完整教程。先说明 username/username 这个特殊仓库的 README 会展示在个人主页，再一步步演示：新建同名仓库、用居中的 HTML 标题加多语言链接实现国际化、用 Skill Icons 嵌入技术栈图标并做深浅色切换。状态统计图给出两种方案：直接改链接即可用的 GitHub Profile Summary Cards，以及带动画但需自建的 GitHub Stats Visualization，后者还详解如何申请个人访问令牌、配置仓库密钥与运行工作流。每步都配有截图，整套流程覆盖从建仓到嵌入图片的每个环节。
---
...
还可以添加多语言版本的链接来实现国际化。当前语言加粗，其他语言保留链接，并用 `·` 分隔。
...
- 标记 `#gh-light-mode-only` 和 `#gh-dark-mode-only` 可以指定某个元素仅在 GitHub 的 **Light mode**（浅色模式）或 **Dark mode**（深色模式）下显示。
