...
一款 C++ 终端小游戏合集，包含多种经典玩法。

## 前言

我在小学四年级第一次接触文本编程，当时使用的 IDE 是 [Dev-C++](https://zh.wikipedia.org/wiki/Dev-C%2B%2B)。在学习算法竞赛的过程中，我萌生了开发游戏的想法，想用 C++ 制作一款大型游戏。相比于图形化编程，终端界面十分简陋，只能输出一些字符。当时的我还不会调用第三方库，只能不断探索各种原生功能，但终端的效果上限却并不低。

iGame 是我从小学五年级开始陆续开发的一款 C++ 终端游戏，当时学校开设了编程社团，我经常和同学一起研究。后来项目就逐渐搁置了，只是偶尔想起才回去改一改，期间还经历过几次重构。这个项目本身并不复杂，如今来看还是有不少「屎山代码」，但它让我探索了很多新的领域。
