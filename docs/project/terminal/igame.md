...
我在小学四年级第一次接触文本编程，当时使用的 IDE 是 [Dev-C++](https://zh.wikipedia.org/wiki/Dev-C%2B%2B)。学习算法竞赛的过程中，我萌生了做游戏的想法，早就想开发一款大型终端游戏。相比于图形化编程，终端界面十分简陋，只能用字符输出和光标控制。当时的我也不会调用第三方库，只能使用各种原生能力，但终端的上限却并不低。

iGame 是我从小学五年级开始陆续开发的一款终端游戏，当时学校开设了编程社团，我经常和同学一起研究。后来就慢慢搁置了，偶尔想起来才回去改一改，中间也经历过几次重构。这个项目本身并不复杂，如今来看还是有不少「屎山代码」，但它让我探索了很多东西。
