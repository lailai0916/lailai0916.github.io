---
title: 地图生成器
description: 本文介绍一种原创地图生成算法及终端实现，灵感来自我的世界，让水与岩浆扩散相遇成边界并逐帧绘制。
summary: 一种原创的地图生成算法及其终端实现。算法灵感来自《我的世界》：在地图上随机撒下水和岩浆，让它们向四周扩散，其中水的流动速度是岩浆的两倍；当水与岩浆相遇时在交界处生成固定的边界，等所有格子停止流动，地图便生成完毕，最终得到被边界自然分割的地形。代码用 C++ 配合 Windows API，在控制台里以不同颜色的色块逐帧绘制水、岩浆与边界，按空格键即可重新生成一张新地图。
---
