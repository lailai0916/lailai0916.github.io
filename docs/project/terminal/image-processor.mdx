---
title: 图片处理器
description: 本文介绍在终端打印图片的工具，用 C++ 逐像素生成 PPM，借 ANSI 真彩色把像素当色块打印。
summary: 在终端里打印图片的一套小工具。图片生成器用 C++ 逐像素写出 PPM（P6 二进制）格式的图像文件，可按算法生成图案；图片显示器读取 PPM，借助 ANSI 转义序列的 24 位真彩色，把每个像素当作两个空格的色块打印到控制台，并对窗口字体、大小与光标做了相应设置。此外给出用 FFmpeg 把连续帧图片合成为视频的安装与命令示例。灵感与思路参考了 matrix67 用极短代码生成图片的文章。整套工具展示了终端在图形输出上的潜力。
---
