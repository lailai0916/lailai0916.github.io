...
我在小学四年级第一次接触文本编程，当时使用的 IDE 还是 [Dev-C++](https://zh.wikipedia.org/wiki/Dev-C%2B%2B)。后来在学习算法竞赛的过程中，我偶然看到一份古老的单文件 C++ 终端游戏《坦克大战》，由此我萌生了自己做一款大型终端游戏的想法。

相比图形化编程，终端界面十分简陋，几乎只能打印字符。在那个没有 AI 的年代，我还不会调用第三方库，只能不断摸索各种原生 API 和控制台特性，遇到问题就查资料反复试错，但终端能做出的效果上限其实并不低。

iGame 是我从五年级开始陆续开发的一款 C++ 终端游戏，当时学校还开设了 Python 编程社团，我经常和同学们一起研究探讨。初中以后项目逐渐搁置了，只有偶尔想起才去改一改。期间经历过几次重构，也精简了很多「屎山代码」。如今来看这个项目本身并不复杂，但它让我探索了很多新东西。
