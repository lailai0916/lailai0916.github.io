---
title: iGame
description: 本文介绍 iGame，一款我断续开发的 C++ 终端游戏合集，含迷宫、双人对战、地图编辑器与五子棋。
summary: lailai 从小学五年级起断续开发的一款 C++ 终端游戏合集。开头讲述项目缘起：受一份单文件终端《坦克大战》启发，在没有 AI、不会调库的年代靠原生功能反复试错，探索终端能做出的效果上限。正文列出待办与历次更新日志，并附上完整源代码，用 Windows API 在控制台里实现了主页按钮、日志系统、随机生成的迷宫与智能寻路、双人对战地图、地图编辑器、五子棋和颜色表等多个模块，也坦言其中不乏当年留下的屎山代码。
---
...
我在小学四年级第一次接触文本编程，当时使用的 IDE 是 [Dev-C++](https://zh.wikipedia.org/wiki/Dev-C%2B%2B)。后来在学习算法竞赛的过程中，我偶然看到一份古老的单文件 C++ 终端《坦克大战》，也让我萌生了自己做一款大型终端游戏的想法。

相比图形化编程，终端界面十分简陋，能输出的只有字符。在那个还没有 AI 的年代，当时的我也不会调用第三方库，只能不断探索各种原生功能，遇到问题就自己查资料反复试错——但终端能做出的效果上限并不低。

iGame 是我从小学五年级开始陆续开发的一款 C++ 终端游戏，当时学校开设了编程社团，我经常和同学一起研究。后来项目就逐渐搁置了，只是偶尔想起才回去改一改，期间还经历过几次重构。这个项目本身并不复杂，如今来看也有不少「屎山代码」，但它让我探索了很多新东西。
