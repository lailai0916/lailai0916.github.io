...
- 通常只使用万能头文件 `<bits/stdc++.h>`，如有特殊需求可补充其他头文件。
...
- 通常只使用标准命名空间 `std`，如有特殊需求可补充其他命名空间。
...
- 模数使用 `mod`，值根据需求设置。
- 无穷大使用 `inf`，值为 `0x3f...` 以便 `memset` 初始化，范围根据需求选择，避免将 `inf` 设为负数。
