...
- 首次出现 **西文术语** 时，应使用 `中文（原文，缩写）` 的格式，例如「快速傅里叶变换（Fast Fourier Transform，FFT）」「世界卫生组织（World Health Organization，WHO）」。
- 首次出现 **外国人名** 时，应使用 `中文（原文，生卒年）` 的格式，例如「艾萨克·牛顿（Isaac Newton，1643–1727）」「艾萨克·牛顿（Isaac Newton，1643–1727）」。
