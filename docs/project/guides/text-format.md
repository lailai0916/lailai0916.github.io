...
- 文件应以 **front matter** 开头，应包含 `title` 和 `description` 字段。
  - `title` 字段应为 **文本标题**，不应使用一级标题。
  - `description` 字段应为 **内容概述**，例如「本文将介绍……」「本文为……」。
- 文件应以 $1$ 个 **空行** 结尾。
- 不应连续使用 $2$ 个空行。
...
- 首次出现 **外国人名** 时，应使用 `中文译名（原文名，生卒年）` 的格式，例如「艾萨克·牛顿（Isaac Newton，1643–1727）」「埃隆·马斯克（Elon Musk）」。
...
- 文本最多使用 $1$ 个 **一级标题**，即文本标题。
- 应使用 **二级、三级、四级标题**，避免使用 **五级、六级标题**。
- 不应跳过标题等级。
- 不应使用相同的上下级标题。
