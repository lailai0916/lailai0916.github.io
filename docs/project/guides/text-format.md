...
- 应使用大小写正确的 **专有名词**，例如「GitHub」「iPhone」。
- 应使用可以读懂的 **缩写和翻译**，不应使用不地道的缩写和翻译，例如 `NBA` ~~`美国职业篮球联赛`~~。
- 同时出现 **中文、原文和缩写** 时，应使用 `中文（原文，缩写）` 的格式，例如「最大公约数（Greatest Common Divisor，GCD）」。
- 减少不必要的 **形容词** 和 **语气词**。
- 应正确使用「的」「地」「得」。
- 应正确使用「他」「她」「它」。
- 应尽量使用 **主动语态**，避免使用被动语态。
...
- Markdown 文本应使用 $2$ <kbd>Space</kbd> 缩进，空行不应使用缩进。
- 每篇文本最多包含 $1$ 个 **一级标题**，其余内容应使用 **二级、三级、四级标题**，避免使用 **五级、六级标题**。
- 标题末尾不应使用标点符号。
- 应使用 **两个星号**（`**`）表示 **加粗**。
- 应使用 **一个下划线**（`_`）表示 **斜体**。
- 应使用 **连字符**（`-`）表示 **无序列表**。
- 行间代码应标记 **代码语言**，纯文本应使用 `text`。
