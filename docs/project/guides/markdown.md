...
**段落** 是由一个或多个连续的 **文本行** 组成，前后要有至少 $1$ 个 **空行**。
...
使用 **井号**（`#`）表示 **标题**，$1\sim 6$ 个井号（`#`）分别对应 $1\sim 6$ 级标题。
...
使用不少于 $3$ 个 **连字符**（`-`）或 **星号**（`*`）表示 **分隔线**。该行不能包含其他内容，但可以插入空格。
