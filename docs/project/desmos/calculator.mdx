---
title: 图形
description: 本文介绍用 Desmos 图形计算器绘制的作品，含凸透镜、细胞膜融合与法国、美国国旗，各附交互演示。
summary: 用 Desmos 图形计算器绘制的几个作品。凸透镜模型由薄透镜成像公式推出单位焦距下像距与物距的关系，再按物距分类讨论成像大小；细胞膜融合借助卡西尼卵形线与伯努利双纽线，取到多个定点距离乘积为定值的点集，并推广到任意多个点；此外还有由色块拼成的法国国旗，以及按《美国法典》国旗标准用色块与多边形绘制的美国国旗。每个作品都附原理推导与可交互的嵌入演示。
---

## 凸透镜模型

一个凸透镜成像演示模型。

### 参考资料

- [透镜 - 维基百科](https://zh.wikipedia.org/wiki/透镜)
- [凸透镜模型 | Desmos](https://www.desmos.com/calculator/547f9a06b9)

### 原理

根据凸透镜的焦距公式：

$$
\frac{1}{f}=\frac{1}{u}+\frac{1}{v}
$$

当焦距 $f$ 为单位长度时：

$$
v=\frac{u}{u-1}
$$

根据物距 $u$ 分类讨论，根据像距 $v$ 计算成像大小。

### 效果展示

<IframeWindow url="https://www.desmos.com/calculator/547f9a06b9?embed" />

## 细胞膜融合

一个神奇的细胞膜融合效果。

### 参考资料

- [用desmos模拟细胞的分裂与融合 - bilibili](https://www.bilibili.com/video/BV1kr42187eE)
- [卡西尼卵形线 - 维基百科](https://zh.wikipedia.org/wiki/卡西尼卵形线)
- [伯努利双纽线 - 维基百科](https://zh.wikipedia.org/wiki/伯努利双纽线)
- [细胞膜融合 | Desmos](https://www.desmos.com/calculator/8e0f85822a)

### 原理

计算到三个定点的距离乘积为定值的点集：

$$
\sqrt{(x-x_1)^2+(y-y_1)^2}\cdot\sqrt{(x-x_2)^2+(y-y_2)^2}\cdot\sqrt{(x-x_3)^2+(y-y_3)^2}=r^3
$$

也可以推广到更多的点：

$$
\prod_{i=1}^n\sqrt{(x-x_i)^2+(y-y_i)^2}=r^n
$$

### 效果展示

<IframeWindow url="https://www.desmos.com/calculator/8e0f85822a?embed" />

## 法国国旗

### 参考资料

- [法国国旗 - 维基百科](https://zh.wikipedia.org/wiki/法国国旗)
- [法国国旗 | Desmos](https://www.desmos.com/calculator/da95805f70)

### 原理

简单的国旗图案，由三个色块组成。

### 效果展示

<IframeWindow url="https://www.desmos.com/calculator/da95805f70?embed" />

## 美国国旗

### 参考资料

- [美国国旗 - 维基百科](https://zh.wikipedia.org/wiki/美国国旗)
- [美国国旗 | Desmos](https://www.desmos.com/calculator/fd87371622)

### 原理

根据《美国法典》（United States Code）中的国旗标准，使用色块和多边形绘制。

### 效果展示

<IframeWindow url="https://www.desmos.com/calculator/fd87371622?embed" />
