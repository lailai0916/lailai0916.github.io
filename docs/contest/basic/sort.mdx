---
title: 排序
description: 本文将介绍十种常见排序算法的复杂度与稳定性对比及手写实现，以及基于比较排序的下限。
summary: 排序算法（Sorting Algorithm）把一组数据按指定顺序排列。一张对比表汇总冒泡、选择、插入、堆、归并、快速、希尔、计数、桶、基数十种排序的平均与最坏时间、空间复杂度、原地与否及稳定性，并指出基于比较的排序最坏下限为 Ω(nlogn)、稳定性即相同键值元素能否保持原有相对次序。随后给出冒泡、选择、插入、归并、快速、计数六种的手写实现，并附 Tim Sort 简介与 std 的 sort、nth_element 等 STL 排序函数。
---
...
|   桶排序（Bucket Sort）    |     $O(n)$     |   $O(n^2)$   |  $O(n+m)$   | Out-of-place |  稳定  |
|   基数排序（Radix Sort）   |    $O(nk)$     |   $O(nk)$    |  $O(n+k)$   | Out-of-place |  稳定  |
...
![](https://cloud.lailai.one/f/kwrUX/sort-intro-1.apng)
...
重复遍历序列，相邻两元素逆序就交换，每轮把当前最大值「冒泡」到末尾。
...
分治：递归排序左右两半，再把两个有序段线性归并为一个有序序列。
...
分治：选一个基准值，把序列划分为不大于与不小于它的两部分，再递归排序。
...
统计每个值的出现次数，再按值域顺序输出，适用于值域较小的整数。
