---
title: 复杂度
description: 本文将介绍时间与空间复杂度的概念、各类渐近符号的含义与性质，以及求解递归式的主定理。
summary: 时间复杂度（Time Complexity）衡量算法用时随数据规模增长的趋势，空间复杂度衡量占用空间的增长趋势。渐近符号里，大 Θ 表示确界、大 O 表示上界、大 Ω 表示下界，小 o 与小 ω 分别是严格上下界，并给出它们的常见运算性质与换底公式带来的底数省略。主定理（Master Theorem）针对形如 T(n)=aT(n/b)+f(n) 的递归式，按 f(n) 与 n 的 log_b(a) 次方的大小关系分三种情形直接读出复杂度，配有多组求解实例。
---
