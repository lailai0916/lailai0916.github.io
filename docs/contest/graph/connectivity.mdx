---
title: 连通性
description: 本文将讲解基于 Tarjan 的连通性，涵盖强连通与双连通分量、割点与桥、圆方树及 2-SAT。
summary: 围绕 Tarjan 的 dfn 与 low 展开一系列连通性问题。强连通分量是有向图中两两互达的极大子图，缩点后原图变为 DAG。无向图的双连通分量分边双与点双，前者删去任一条边仍连通，后者删去任一个点仍连通。割点是删去后连通分量增多的点，桥则是删去后连通分量增多的边。圆方树把点双结构转成一棵树，便于处理仙人掌。2-SAT 把每个变量拆成真假两点建蕴含图，跑 SCC 判断可满足性并构造赋值。各部分均附参考代码。
---
