---
title: 旋转卡壳
description: 本文将介绍求凸包直径即平面最远点对的旋转卡壳，用两条平行卡壳枚举对踵点，附凸包求法。
summary: 旋转卡壳用来计算凸包直径，也就是平面最远点对的距离。关键结论是最远点对一定落在凸包上，且互为一对对踵点。做法先求出凸包，再用两条平行卡壳贴着凸包同向旋转一周，沿途枚举所有对踵点对并取最大值，这一步为线性。给出以 Andrew 与 Graham 两种方式求凸包的完整参考代码，均用叉积比较三角形面积来推进对踵点指针，最终输出最远距离的平方。文末附一道模板例题。
---
...
**旋转卡壳** 可以用来计算凸包直径，即平面最远点对的距离。最远点对一定是凸包上的一对对踵点，于是先求出凸包，再用两条平行卡壳贴着凸包同向旋转一周，沿途枚举所有对踵点对取最大值，这一步为 $O(n)$。下面先用 Andrew 或 Graham 求出凸包。
