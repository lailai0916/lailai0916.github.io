---
title: 距离
description: 本文将介绍度量的三条公理，及欧几里得、曼哈顿、切比雪夫距离与其相互转化，并推广到闵可夫斯基距离。
summary: 从度量的三条公理讲起：同一性、对称性、三角不等式，并由它们推出非负性。随后给出三种常见距离——欧几里得距离由勾股定理得来，曼哈顿距离是坐标差绝对值之和，切比雪夫距离是坐标差绝对值的最大值。两者可以相互转化：把点 (x,y) 变为 (x+y,x-y) 后，原曼哈顿距离等于新切比雪夫距离，反向缩放一半则相反。最后推广到闵可夫斯基距离，参数 p 取 1、2、无穷时分别退化为上述三种，且 p 不小于 1 时才构成度量。
---
...
在数学（尤其是拓扑学、几何学）中，**度量** 是指一种满足以下三条性质的「距离函数」：
