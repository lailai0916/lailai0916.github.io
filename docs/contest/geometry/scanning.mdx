---
title: 扫描线
description: 本文将介绍扫描线，以矩形面积并为例，用事件线配合线段树维护覆盖长度，把二维统计降为一维。
summary: 扫描线是把二维问题降为一维动态问题的思想：用一条假想的线沿某轴方向扫过，每到一个关键位置就更新数据结构，从而统计面积、周长或点数。以矩形面积并为例，把每个矩形拆成下边加一、上边减一的两条竖直事件线，按 x 排序后逐步推进，每一步查询 y 方向被覆盖的总长度乘以横向步长再累加。y 区间用一棵特殊线段树维护，每个节点存覆盖次数 cnt 与有效长度 len，配合坐标离散化，复杂度 O(nlogn)。
---
