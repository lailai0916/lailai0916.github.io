---
title: 珂朵莉树
description: 本文将介绍珂朵莉树，用 set 合并同值区间、以 split 与区间赋值维护随机数据的区间操作。
summary: 珂朵莉树又名老司机树（ODT），起源于 CF896C，核心是把值相同的连续区间合并成一个节点存进 set，靠数据随机时区间数很少来保证均摊效率。关键操作是 split：把包含 x 的区间从 x 处劈开并返回右段迭代器；区间赋值 cover 借助两次 split 后 erase 掉整段再插入新节点，把区间数压下来，区间加与区间求和则遍历涉及的节点。提醒求区间端点时必须先 split 右端点再 split 左端点，否则迭代器可能失效导致 RE。例题为 CF896C。
---
