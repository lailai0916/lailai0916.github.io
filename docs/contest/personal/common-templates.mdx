---
title: 常用模板
description: 本文为 lailai 的 OI 常用代码模板，涵盖读写、算法基础、字符串、数学、数据结构与图论。
summary: lailai 在 OI 中的常用代码模板汇总。通用部分给出基础与快捷主程序框架、计时性能测试、联赛文件结构；读写部分含文件重定向、关闭同步、多种快读快写、cout 格式化与缓冲刷新。此外按主题整理了算法基础（归并排序、整数与实数二分）、字符串（KMP、字典树）、数学（快速幂、排列组合、欧拉筛、随机数、矩阵、复数、分数）、数据结构（并查集、ST 表、树状数组、线段树、平衡树）、图论（树链剖分、最近公共祖先）与计算几何的模板。
---
...
- [ACM 常用算法模板 - kuangbin](https://acm.sdut.edu.cn/10th_anniversary_celebration/kuangbin的ACM模板（新）.pdf)
...
联赛文件结构模板：[name.zip](https://cloud.lailai.one/s/DOT3)
...
详见 [读写优化](/docs/contest/contest/io)。
...
详见 [二分](/docs/contest/basic/binary)。
...
详见 [字符串匹配](/docs/contest/string/match)。
...
详见 [字典树](/docs/contest/string/trie)。
...
详见 [快速幂](/docs/contest/math/binary-exponentiation)。
...
详见 [排列组合](/docs/contest/math/combinatorics/permutation-and-combination)。
...
详见 [筛法](/docs/contest/math/number-theory/sieve)。
...
详见 [矩阵](/docs/contest/math/linear-algebra/matrix)。
...
详见 [树状数组](/docs/contest/ds/fenwick)。
...
详见 [树链剖分](/docs/contest/graph/tree/hld)。
