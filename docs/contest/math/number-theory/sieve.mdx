---
title: 筛法
description: 本文将介绍埃氏筛与线性欧拉筛，以及用欧拉筛递推欧拉函数、莫比乌斯函数等积性函数的方法。
summary: 筛法用于求素数与积性函数。埃拉托斯特尼筛法从每个素数出发把它的倍数标记为合数，时间复杂度为 O(n log log n)；欧拉筛法即线性筛，让每个合数只被它的最小质因子筛去一次，达到 O(n)，并能在筛的过程中递推积性函数。给出欧拉筛递推欧拉函数、莫比乌斯函数、约数个数函数与约数和函数四种实现，各自按当前数是否被最小质因子整除分两种转移。开头另附三种算法的复杂度对比表。
---

## 参考资料

- [筛法 - OI Wiki](https://oi-wiki.org/math/number-theory/sieve/)
- [埃拉托斯特尼筛法 - 维基百科](https://zh.wikipedia.org/wiki/埃拉托斯特尼筛法)

## 算法对比

|     算法名称     |    时间复杂度    |
| :--------------: | :--------------: |
|     朴素算法     |  $O(n\sqrt n)$   |
| 埃拉托斯特尼筛法 | $O(n\log\log n)$ |
|     欧拉筛法     |      $O(n)$      |

## 埃拉托斯特尼筛法

埃拉托斯特尼筛法（Sieve of Eratosthenes）简称埃氏筛法。

```cpp
void sieve()
{
	vis[0]=vis[1]=1;
	for(int i=2;i*i<N;i++)
	{
		if(vis[i])continue;
		for(int j=i*i;j<N;j+=i)vis[j]=1;
	}
	int cnt=0;
	for(int i=2;i<N;i++)
	{
		if(vis[i])continue;
		pri[++cnt]=i;
	}
}
```

## 欧拉筛法

欧拉筛法（Sieve of Euler）也称线性筛法，让每个合数只被它的最小质因子筛去一次，时间复杂度为 $O(n)$。它还能在筛的过程中递推出欧拉函数、莫比乌斯函数等积性函数。

### 素数

```cpp
void sieve()
{
	vis[0]=vis[1]=1;
	int cnt=0;
	for(int i=2;i<N;i++)
	{
		if(!vis[i])pri[++cnt]=i;
		for(int j=1;j<=cnt;j++)
		{
			if(i*pri[j]>=N)break;
			vis[i*pri[j]]=1;
			if(i%pri[j]==0)break;
		}
	}
}
```

### 数论函数

<Tabs>
<TabItem value="欧拉函数">

$$
\varphi(n)=n\prod_{p\mid n}\frac{p-1}{p}
$$

```cpp
void sieve()
{
	vis[0]=vis[1]=1;
	phi[1]=1;
	int cnt=0;
	for(int i=2;i<N;i++)
	{
		if(!vis[i])
		{
			pri[++cnt]=i;
			phi[i]=i-1;
		}
		for(int j=1;j<=cnt;j++)
		{
			if(i*pri[j]>=N)break;
			vis[i*pri[j]]=1;
			if(i%pri[j]==0)
			{
				phi[i*pri[j]]=phi[i]*pri[j];
				break;
			}
			phi[i*pri[j]]=phi[i]*phi[pri[j]];
		}
	}
}
```

</TabItem>
<TabItem value="莫比乌斯函数">

$$
\mu(n)=
\begin{cases}
  1 & n=1 \\
  0 & \exists d>1,d^2\mid n \\
  (-1)^{\omega(n)} & \text{otherwise}
\end{cases}
$$

```cpp
void sieve()
{
	vis[0]=vis[1]=1;
	mu[1]=1;
	int cnt=0;
	for(int i=2;i<N;i++)
	{
		if(!vis[i])
		{
			pri[++cnt]=i;
			mu[i]=-1;
		}
		for(int j=1;j<=cnt;j++)
		{
			if(i*pri[j]>=N)break;
			vis[i*pri[j]]=1;
			if(i%pri[j]==0)
			{
				mu[i*pri[j]]=0;
				break;
			}
			mu[i*pri[j]]=-mu[i];
		}
	}
}
```

</TabItem>
<TabItem value="约数个数函数">

$$
d(n)=\sum_{d\mid n}1=\prod_{i=1}^k(a_i+1)
$$

```cpp
void sieve()
{
	vis[0]=vis[1]=1;
	num[1]=d[1]=1;
	int cnt=0;
	for(int i=2;i<N;i++)
	{
		if(!vis[i])
		{
			pri[++cnt]=i;
			num[i]=d[i]=2;
		}
		for(int j=1;j<=cnt;j++)
		{
			if(i*pri[j]>=N)break;
			vis[i*pri[j]]=1;
			if(i%pri[j]==0)
			{
				num[i*pri[j]]=num[i]+1;
				d[i*pri[j]]=d[i]/num[i]*num[i*pri[j]];
				break;
			}
			num[i*pri[j]]=2;
			d[i*pri[j]]=d[i]*2;
		}
	}
}
```

</TabItem>
<TabItem value="约数和函数">

$$
\sigma(n)=\sum_{d\mid n}d=\prod_{i=1}^k\left(1+p_i+p_i^2+\dots+p_i^{a_i}\right)
$$

```cpp
void sieve()
{
	vis[0]=vis[1]=1;
	num[1]=sigma[1]=1;
	int cnt=0;
	for(int i=2;i<N;i++)
	{
		if(!vis[i])
		{
			pri[++cnt]=i;
			num[i]=sigma[i]=i+1;
		}
		for(int j=1;j<=cnt;j++)
		{
			if(i*pri[j]>=N)break;
			vis[i*pri[j]]=1;
			if(i%pri[j]==0)
			{
				num[i*pri[j]]=num[i]*pri[j]+1;
				sigma[i*pri[j]]=sigma[i]/num[i]*num[i*pri[j]];
				break;
			}
			num[i*pri[j]]=pri[j]+1;
			sigma[i*pri[j]]=sigma[i]*sigma[pri[j]];
		}
	}
}
```

</TabItem>
</Tabs>

## 例题

<Problem id="P3383" />

<Problem id="P1865" />
