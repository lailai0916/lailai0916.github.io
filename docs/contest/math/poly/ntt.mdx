...
[快速傅里叶变换（FFT）](/docs/contest/math/poly/fft) 在复数域上用单位根 $\omega_n=e^{2\pi i/n}$ 做变换，浮点运算存在精度误差。**快速数论变换**（Fast Number Theoretic Transform，FNTT）把整个过程搬到模 $p$ 的整数域上，用 **原根**（Primitive Root）的幂代替单位根，既精确又天然适合带取模的题目。
