---
title: 标准库
description: 本文将介绍 C++ STL 常用序列式与关联式容器、适配器及 sort 等算法。
summary: 标准模板库（Standard Template Library，STL）是 C++ 标准库的一部分，提供一批模板化的通用数据结构与算法，是竞赛编码的基础。文中按类别梳理常用容器：序列式容器有 array、vector、deque、list、forward_list；关联式容器有基于红黑树、元素有序的 set、multiset、map、multimap，以及以哈希实现、平均常数级操作的对应无序版本；容器适配器有 stack、queue、priority_queue；此外还有 bitset、string、pair、tuple 等并不严格属于容器体系的类型。对 deque、stack、queue 逐一列出访问、修改、容量三类常用成员函数，最后汇总 sort、lower_bound、next_permutation 等高频算法，这些算法大多以迭代器为接口，能够作用于不同的容器之上。
---
...
![](https://cloud.lailai.one/f/D6kUK/container.png)
...
<Quote author="Walski Schölder">The "ueue" in "queue" is silent, but not in "deque."</Quote>
