---
title: 赛事 & 题型
description: 本文将介绍 OI 与 ICPC 两大赛事体系、四种赛制差异及传统题评测结果与题型。
summary: 梳理算法竞赛的赛事、赛制与题型。赛事部分区分 OI 与 ICPC/CCPC 两大体系及其起源，赛制部分对比 OI、IOI、ICPC 与 Codeforces 四种规则在提交次数、实时反馈、罚时和 Hack 机制上的差异。题型部分列出传统题的各类评测结果，如 AC、CE、WA、TLE、MLE 等缩写的含义，并说明交互题、通信题以及输出自身源代码的自产生程序（Quine）各自的特点。
---
...
信息学奥林匹克竞赛（Olympiad in Informatics，OI）是一门在中学生中广泛开展的学科竞赛，和物理、数学等竞赛性质相同；该竞赛于 1984 年在中国起源，属中国高中五大学科竞赛之一。OI 考查的内容是参赛者运用算法、数据结构和数学知识，通过编写计算机程序解决实际问题的能力。
