---
title: 背包 DP
description: 本文将介绍背包 DP 的 0-1、完全、多重三种基本模型及其转移，含二进制分组优化的多重背包。
summary: 背包 DP（Knapsack DP）在总重量不超过容量的前提下最大化所选物品的总价值，分 0-1、完全、多重三种基本模型。0-1 背包中每件物品至多取一次，倒序枚举容量，转移取选与不选的较大值；完全背包中每种物品可取任意多次，改为正序枚举容量即可复用同一状态；多重背包中每种物品有固定件数，借助二进制分组把它拆成不超过 log k 个「捆绑」物品，再当作 0-1 背包处理。三种模型均给出转移方程与参考代码。
---
