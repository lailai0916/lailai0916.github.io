---
title: 可持久化数据结构
description: 本文将介绍保留历史版本的可持久化线段树，讲解复制访问路径共享节点的主席树及其例题。
summary: 可持久化数据结构保留每次操作后的历史版本。可持久化线段树每次修改只复制访问路径上的节点、得到新版本并与旧版共享未改部分，从而以对数级空间保存各版本状态，常用于静态区间查询与离线的版本查询。因引入者姓名缩写与某位国家主席相同，故又称主席树。给出支持单点修改与历史版本单点查询的实现，节点存左右儿子与值，root 数组记录各版本根。附可持久化数组与静态区间第 k 小两道例题。
---
