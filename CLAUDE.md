...
- **`code.json` must follow Docusaurus's canonical (auto-generated) order.** That means: the order produced when you delete `code.json` and run `npm run i18n` from a clean state. Do not hand-reorder, group, or alphabetize — the file is the merge of "fresh write-translations output" + "your existing Chinese messages preserved per key." When introducing new keys, just run `npm run i18n` and they land in the right slot automatically. (`npm run check` runs `i18n` first, so the order is enforced on every commit.)
