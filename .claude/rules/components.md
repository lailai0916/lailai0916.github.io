...
- **No decorative gradients or translucent fills.** Per `design-style.md`. Slider progress is a flat state fill using track/fill layers, not a background gradient. The only gradient in `laikit` is functional: the Skeleton shimmer. Tinted surfaces (e.g. `IconBlock` accent) are flat `color-mix()`, not gradients. (The Problem header's `backdrop-filter` frosted glass and the About greeting's wordmark are deliberate, isolated exceptions.)
