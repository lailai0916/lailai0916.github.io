...
- **Atoms / display:** `Badge`, `IconBlock`, `IconText`, `Tooltip`, `Skeleton`, `TrafficLights`, `WindowBar`, `Markdown`, `Quote`, `GitHub`. `WindowBar` is the one mac-style window title bar (frosted 42px strip led by `TrafficLights`); callers pass their own grid layout via `className` and the content after the dots. It is the single source for that chrome — `BrowserWindow` (address bar), `Problem` / playground `Showcase` (tab strip), and the swizzled `CodeBlock` header all render it. Don't re-hand-roll the frosted bar; reuse `WindowBar`.
...
## Shared article chrome (blog + docs)
...
- **Interaction transitions are `160ms ease`.** Hover/active/color/border transitions all use `160ms ease` (always name the property — never `transition: 0.3s`, which animates `all`). Longer, deliberately different durations are reserved for content reveals / loaders (e.g. Skeleton shimmer, the MetricList bar-fill) — those are a separate category, not interaction feedback.
- **No decorative gradients or translucent fills.** Per `design-style.md`. Slider progress is a flat state fill using track/fill layers, not a background gradient. The only gradient in `laikit` is functional: the Skeleton shimmer. Tinted surfaces (e.g. `IconBlock` accent) are flat `color-mix()`, not gradients. (The `WindowBar` frosted glass — `backdrop-filter` + a flat two-stop tint, the shared window chrome — and the About greeting's wordmark are deliberate, isolated exceptions.)
- **Elevation & focus go through tokens, never hardcoded.** `src/css/custom.css` owns one theme-aware scale — `--lk-shadow-{sm,raised,md,lg,overlay}` (layered ambient + key shadows; each has a deeper `[data-theme='dark']` variant so dark surfaces actually read as elevated) and `--lk-focus-ring` (the single `0 0 0 3px` primary ring, 22% light / 30% dark). Surfaces map by role: resting card = `md`, raised tile-in-track (Segmented/Problem active tab) = `raised`, overlay popover (Tooltip) = `lg`, full sheet (CookieConsent) = `overlay`. Any `:focus-visible` ring is `var(--lk-focus-ring)`. Don't reintroduce a per-component `box-shadow: 0 …` literal or a bespoke dark-shadow override — add to / pick from the scale instead. (Only genuinely bespoke shadows stay inline: `TrafficLights`' inset hairline and Slider's thumb hairline.)
...
- **Shared measured-height animation** lives in `useMeasuredHeight` (`src/hooks/`) — reused by `Problem` and the playground `Showcase`. It measures the content (re-measuring on the passed key, e.g. the active tab, and on resize); the caller drives a `.viewport` wrapper's height from it with a `transition: height` so collapse/expand and tab switches animate to the real height (no fixed height, no reserved dead space). Don't hand-roll a `ResizeObserver` + collapse for a new tabbed window panel — reuse the hook.
- **Motion respects `prefers-reduced-motion` globally.** `src/css/custom.css` carries one guard that neutralizes animations/transitions site-wide under the OS setting, so new **CSS** animations need no per-component handling. **JS-driven** motion (the home typewriter, travel globe `autoRotate`, settings confetti) must gate itself — on `window.matchMedia('(prefers-reduced-motion: reduce)').matches`, or the library's own flag (canvas-confetti's `disableForReducedMotion`). Caveat: a keyframe that ends hidden (`opacity: 0`, like the home `.roleCursor` blink) needs its own co-located `@media (prefers-reduced-motion: reduce) { animation: none }`, or the global guard freezes it on the invisible final frame.
...
- **Shared-module pattern:** when a few classes are genuinely shared across sibling components, keep a small root `styles.module.css` and import it as `shared` alongside the component's own `styles`. The real cases today are `src/theme/BlogShared/styles.module.css` (theme-toggled cover images) and `src/components/Article/styles.module.css` (the `.metaItem` / `.metaLink` / `.iconBtn` action-icon buttons shared by `Article/Actions`, `Article/CopyMarkdownButton`, `Article/MetaFooter`). Component-exclusive classes stay in the component's own module. Do not let a shared module accrete single-owner classes.
...
**Explicitly-imported page demos** (not registered): `src/components/Countdown` (a birthday countdown `Card`) and `src/components/playground/*` (`FourierTransform`, `GameOfLife`, `LorenzAttractor`, `NeuralNetwork`, `NimGame` — interactive `<canvas>`/React demos) are one-off widgets imported by `import` in the MDX that uses them — today the `docs/project/index.mdx` landing page (plus one playground demo in `docs/note/english/vocabulary/gaokao.mdx`). They are page showcases, not reusable primitives; keep new playground demos under `playground/` and import them where used rather than registering them globally. Each demo self-frames in a laikit `Card` by default but takes a `bare` prop to swap that Card for a light hairline frame (no shadow) via the shared `playground/Surface` wrapper. `playground/Showcase` composes all five into one browser-window panel — tabs live in the title bar via the shared `laikit/WindowBar` chrome (same as `Problem`), and the active demo renders `bare` in the collapsible body; `docs/project/index.mdx` renders just `<Showcase />` under `## 游乐场` instead of stacking the demos under `###` headings. A new playground demo should render its outer frame through `Surface` and be added to `Showcase`'s `DEMOS` list.

## Theme overrides

Swizzled Docusaurus overrides live in `src/theme/` (Layout, Blog\*, MDXComponents, Admonition, Root, `DocPaginator`, `NotFound/Content`, `DocCard` + `DocCardList`, `DocItem/Layout` + `DocItem/Footer`). Use `npm run swizzle` rather than hand-copying framework files. A swizzled override reuses its own framework `theme.*` strings (e.g. `DocPaginator` keeps `theme.docs.paginator.*`); genuinely new strings take a project prefix (`NotFound/Content`'s home button is `components.notFound.*`, mirroring the `Root/CookieConsent` → `components.cookieConsent.*` precedent). The `DocItem/*` overrides carry no strings of their own — they delegate to the shared `src/components/Article/` chrome (Actions top-right, MetaFooter at the bottom) so docs and blog match. `Root` renders two client-only siblings inside `BrowserOnly` — `CookieConsent` and `ThemeColor` (the latter keeps `<meta name="theme-color">` synced to the resolved `--ifm-background-color`, re-running on `data-theme` changes so the mobile browser chrome tracks the manual light/dark toggle). `src/theme/BlogShared/` is **not** an official Docusaurus slot — it is a local helper folder of blog building blocks (`PostChrome`, `BlogUI`, `Scaffold`, etc.), each following the same per-component-folder layout as `laikit`; the article-chrome bits it used to own now live in `src/components/Article/`.
