...
- Dates are `YYYY-MM-DDTHH:MM:SS` (wall-clock Asia/Shanghai, no timezone): `date: 2025-08-04T15:30:00`. Every blog post carries seconds. **Solution posts** (`blog/solution/*.mdx`) use the **exact** publish second of their Luogu column counterpart (the `aid` in `<Solution>`; Luogu records second precision) — keep the two in sync. All other posts use `:00` for the seconds.
...
- Solution posts replace the lead with the `<Solution pid="..." aid="..." />` widget, then `{/* truncate */}`. No prose before the widget.
...
- Range/interval in math contexts is `\sim`: `$09:00\sim 11:00$`, `$625\sim 627$`. Never `~` ASCII.
- Wrap **any numeric quantity** in inline math, even in narrative prose: `$30$ 分`, `$2$ 月 $26$ 日`, `$616$ 分`. This is a consistent author tic — don't strip it.
...
- **`<Solution pid="..." aid="..." />`** — first non-frontmatter line of every `blog/solution/*.mdx`, before `{/* truncate */}`. Exactly two attrs.
