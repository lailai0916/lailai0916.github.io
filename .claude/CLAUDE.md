...
- `src/components/` (non-laikit) — MDX-only widgets exposed to authors (`BrowserWindow`, `Desmos`, `Notation`, `Problem`, `Solution`), plus `Article/` — the article-page chrome (`Actions`, `CopyMarkdownButton`, `MetaFooter`) shared by blog posts and the swizzled docs `DocItem/*`.
