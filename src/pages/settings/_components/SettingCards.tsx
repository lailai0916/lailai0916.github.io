...
export function ColorGenerator() {
  ...
  const [inputColor, setInputColor] = useState(isDarkTheme ? DARK_PRIMARY_COLOR : LIGHT_PRIMARY_COLOR);
  const [baseColor, setBaseColor] = useState(isDarkTheme ? DARK_PRIMARY_COLOR : LIGHT_PRIMARY_COLOR);
  const [background, setBackground] = useState(isDarkTheme ? DARK_BACKGROUND_COLOR : LIGHT_BACKGROUND_COLOR);
  const [shades, setShades] = useState(COLOR_SHADES);
  const [storage, setStorage] = useState(isDarkTheme ? darkStorage : lightStorage);

  const presetColors = ["#1d9bf0", "#ffd400", "#f91880", "#7856ff", "#ff7a00", "#00ba7c"];
  ...
  useEffect(() => {
    const DEFAULT_PRIMARY_COLOR = isDarkTheme ? DARK_PRIMARY_COLOR : LIGHT_PRIMARY_COLOR;
    const DEFAULT_BACKGROUND_COLOR = isDarkTheme ? DARK_BACKGROUND_COLOR : LIGHT_BACKGROUND_COLOR;
  ...
  return (
    <SettingCard title="主题色生成器" icon="lucide:palette">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(to right, ${getAdjustedColors(shades, baseColor)
