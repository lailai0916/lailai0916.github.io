  ...
  return (
    <BaseCard isClickable={false} className="p-5 items-center justify-center">
      <div className="flex flex-col items-center space-y-3">
        <IconWrapper>
          <img src={icon} alt={title} draggable="false" className="w-10 h-10 object-contain filter brightness-0 dark:invert transition-transform duration-200 select-none" />
        </IconWrapper>
        <h3 className={`font-medium text-base ${TEXT_COLORS.PRIMARY} text-center leading-snug`}>{title}</h3>
      </div>
    </BaseCard>
  );
}
