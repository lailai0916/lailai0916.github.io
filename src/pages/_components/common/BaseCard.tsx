...
function CardContent({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <article className={`${CARD_BASE_STYLES} ${className}`}>{children}</article>;
}

export default function BaseCard({ children, href, className = '', isClickable = true, isExternalLink = false, onClick }: BaseCardProps) {
  // 非交互式卡片
  if (!isClickable) {
  ...
  if (href) {
    ...
    const linkProps = isExternalLink ? { href, target: '_blank', rel: 'noopener noreferrer' } : { to: href };

    return (
      <LinkComponent {...linkProps} className={LINK_STYLES} style={FOCUS_STYLES} {...focusHandlers}>
  ...
  if (onClick) {
    return (
      <button onClick={onClick} className={`${LINK_STYLES} cursor-pointer`} style={FOCUS_STYLES} {...focusHandlers}>
        <CardContent className={className}>{children}</CardContent>
      </button>
    );
  }
