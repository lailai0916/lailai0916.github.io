...
export default function BlogAuthorsListPage(props: Props) {
  const {authors} = props;
  return (
    <BlogScaffold title="Authors" left={<SidebarLeft />} right={<SidebarRight />}>
      <div className={styles.card}>
        <div className={styles.cardTitle}><Translate id="blog.authors">Authors</Translate></div>
        <div className={styles.tagList}>
          {authors.map((a: any) => (
            <Link key={a.permalink} to={a.permalink} className={styles.tagChip}>
              <span className={styles.tagDot} />{a.name}
