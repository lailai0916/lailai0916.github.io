...
function LeftSidebar() {
  ...
  const author = (metadata.authors ?? []).find((a: any) => (a?.key || a?.name) === AUTHOR_ID) || null;
        ...
        <div className={styles.tagList}>
          {getTopTags(12).map((t) => (
            <Link key={t.permalink} to={t.permalink} className={styles.tagChip}>
              <span className={styles.tagDot} />{t.label}<span className={styles.tagCount}>{t.count}</span>
            </Link>
          ))}
        </div>
      </div>
...
export default function BlogPostPage(props: Props): ReactNode {
  ...
  return (
    <BlogPostProvider content={props.content} isBlogPostPage>
      <HtmlClassNameProvider className={`${ThemeClassNames.wrapper.blogPages} ${ThemeClassNames.page.blogPostPage}`}>
        <BlogPostPageMetadata />
        <BlogPostPageStructuredData />
        <BlogScaffold title={props.content.metadata.title} description={props.content.metadata.description} left={<LeftSidebar />} right={<SidebarRight />}>
          <BlogPostItem>
            <BlogPostContent />
          </BlogPostItem>
          {(props.content.metadata.nextItem || props.content.metadata.prevItem) && (
            <BlogPostPaginator nextItem={props.content.metadata.nextItem} prevItem={props.content.metadata.prevItem} />
          )}
        </BlogScaffold>
