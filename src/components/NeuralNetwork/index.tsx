/*
  Neural Network Interactive Component
  Originally created by Grant Sanderson (3Blue1Brown)
  Adapted for Docusaurus/TypeScript environment
  ...
  const normalizePointsAnimated = (duration = 1.0) => {
    ...
    return new Promise((resolve) => {
      const frame = () => {
        ...
        const newPoints = applyNormalizationTransformation(points, data, ease(Math.min(1, t / duration)));
        setPoints(newPoints);
    ...
    >
      ...
      {selectedNeuron &&
        ...
        (() => {
          ...
          return <WeightGrid x={position.x + 20} y={-40 + (position.y - 240) * 0.85 + 240} width={85} height={85} weights={weights[0][selectedNeuron.neuronId]} inputNeurons={neurons[0]} />;
        })()}
      ...
      <ImageGrid
        ...
        highlightedTile={selectedNeuron && selectedNeuron.layerIndex === 0 ? selectedNeuron.neuronId : null}
      />
    </svg>
  );
}
  ...
  for (let layerIndex = 1; layerIndex < visibleNeurons.length; layerIndex++) {
    ...
    layer.forEach((neuronId, neuronIndex) => {
      ...
      prevLayer.forEach((prevNeuronId, prevNeuronIndex) => {
        ...
        const prevNeuronPos = getNeuronPosition(prevLayerIndex, prevNeuronIndex);
        const nextNeuronPos = getNeuronPosition(layerIndex, neuronIndex);

        const lineLength = Math.hypot(prevNeuronPos.x - nextNeuronPos.x, prevNeuronPos.y - nextNeuronPos.y);

        const thisLineCanAnimate = (prevNeuronId * layer.length + neuronId) % 7 === 2;

        connections.push(<line key={`${prevLayerIndex}-${prevNeuronId}-${layerIndex}-${neuronId}`} x1={prevNeuronPos.x} x2={nextNeuronPos.x} y1={prevNeuronPos.y} y2={nextNeuronPos.y} stroke={color} strokeWidth={lineWidth} />);
        ...
        if (thisLineCanAnimate && !instant) {
          connections.push(
            <line
              ...
              style={{
                transition: animating ? `stroke-dashoffset 1200ms ease-in-out ${1200 * (layerIndex - 1) + 500 + 100 * Math.random()}ms` : 'none',
              }}
            />
          );
        }
...
function Neurons({ neurons, selectedNeuron, setSelectedNeuron, animating, instant }) {
  return (
    <g>
      {visibleNeurons.map((layer, layerIndex) =>
        layer.map((neuronId, neuronIndex) => {
          ...
          const isSelected = selectedNeuron && selectedNeuron.layerIndex === layerIndex && selectedNeuron.neuronId === neuronId;
          ...
          return (
            <circle
              ...
              style={{
                ...
                transition: animating && !instant ? `fill 600ms ease-in-out ${1200 * layerIndex + 100}ms` : 'none',
                cursor: 'pointer',
              }}
              onClick={() => {
                if (isSelected) {
...
function WinningOutputNeuronBox({ neurons, animating, instant }) {
  ...
  return (
    <rect
      ...
      style={{
        transition: animating ? `stroke-dashoffset 800ms ease-in-out ${instant ? '0ms' : '4500ms'}` : 'none',
      }}
    />
  );
}
...
function OutputDigitLabels() {
  return (
    <g>
      {visibleNeurons[visibleNeurons.length - 1].map((neuronId, neuronIndex) => {
        const position = getNeuronPosition(visibleNeurons.length - 1, neuronIndex);

        return (
          <text key={neuronId} x={position.x + 25} y={position.y + 2} style={{ fill: 'white' }} fontSize="20" dominantBaseline="middle" textAnchor="middle">
            {neuronId}
          </text>
        );
      })}
...
function ImageGrid({ x, y, width, height, points, setPoints, normalizing, isNormalized, normalizePointsAnimated, instant, editing, startEditing, beginAnimation, highlightedTile }) {
  ...
  const onMouseDown = useCallback(
    (event) => {
      if (editing) {
        setDragging(true);
        fillAtEventLocation(event, false);
  ...
  return (
    <g
      style={{
        ...
        willChange: editing ? 'auto' : 'transform',
      }}
    >
      ...
      <g>
        {values.map((value, n) => {
          ...
          return <rect key={`${tileX}-${tileY}`} x={(tileX * 400) / 28} y={(tileY * 400) / 28} width={400 / 28} height={400 / 28} fill={`rgba(255, 255, 255, ${value})`} stroke={highlightedTile === n ? 'yellow' : 'none'} strokeWidth="2" />;
        })}
      </g>
      ...
      >
        ...
        <text x={200} y={50} dominantBaseline="middle" textAnchor="middle" fill="yellow" fontFamily="sans-serif" fontSize="36">
          Pre-processing...
        </text>
      </g>
...
function WeightGrid({ x, y, width, height, weights, inputNeurons }) {
  ...
  return (
    <g transform={`translate(${x} ${y}) scale(${width / 28} ${height / 28})`}>
      <rect x="-1" y="-1" width="30" height="30" fill="black" stroke="yellow" strokeWidth="0.5" />
...
function getAllNeuronValues(firstLayer) {
  ...
  while (layers.length <= weights.length) {
    ...
    layers.push(vectorAdd(matrixVectorMult(weightMatrix, previousLayer), biasVector).map(sigmoid));
  }
