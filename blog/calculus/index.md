...
如果每次都从 $A_a$ 累加到 $A_b$，需要运算很多次，效率较低。
...
定积分 $\displaystyle\int_a^b f(x)\mathrm{d}x$ 表示函数 $f(x)$ 图像在区间 $[a,b]$ 上与 $x$ 轴所围成的 **有向面积**：
...
整个符号可以理解为：从 $a$ 到 $b$ 将 **无穷多个** 宽度为 $\mathrm{d}x$ 的细长方形面积累加起来，每个细长方形的长为 $f(x)$。
...
类似地，对于 **函数** $f(x)$，我们也可以求“前缀和”，从某个 **固定点** $c$ 开始，计算函数 $f(x)$ 在区间 $[c,x]$ 的面积，记为 **原函数** $F(x)$：
...
这个类似“前缀和”过程叫做 **积分**，由于 $c$ 可以是任意常数，所以原函数有 **无穷多个**。**不定积分** $\int f(x)dx$ 表示所有可能的原函数，它们之间相差一个常数 $C$，这就是 **积分常数**：
...
| :-----------------: | :---------------------------------------------: | :----------------------------------------------------: |
|        对象         |                      $A_n$                      |                         $f(x)$                         |
| 区间 $[a,b]$ 的“和” |   $T=\displaystyle\sum_{i=a}^b A_i$<br />求和   | $S=\displaystyle\int_a^b f(x)\mathrm{d}x$<br />定积分  |
...
刚才我们通过模仿数列前缀和的思路，找到了计算定积分的方法：先构造原函数，再计算两点的差值。

$$
F(x)=\displaystyle\int_c^x f(t)\mathrm{d}t
$$

思考：具体如何计算函数 $f(x)$ 的原函数 $F(x)$ 呢？
