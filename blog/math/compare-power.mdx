---
title: 数学：比较 x^y 和 y^x
description: 本文将介绍如何比较正实数 x 的 y 次与 y 的 x 次谁大，借助函数 ln t 除以 t 讨论。
summary: 比较两个正实数 x 的 y 次与 y 的 x 次谁大。两边取自然对数，化为比较 ln x 除以 x 与 ln y 除以 y，从而构造函数 f(t) 等于 ln t 除以 t。求导得知它在 t 等于 e 处取最大值，之前递增、之后递减。据此按 x、y 落在 e 的同侧还是两侧分类讨论，并给出 2 的 4 次等于 4 的平方、e 的 π 次大于 π 的 e 次两个例子。
date: 2025-08-23T15:17:00+08:00
authors: lailai
tags: [math]
---
