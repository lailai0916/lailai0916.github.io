---
title: 数学：微积分入门指南
description: 本文将介绍面向中学生的微积分入门，从前缀和类比讲到定积分、不定积分与牛顿–莱布尼茨公式。
summary: 面向中学生的微积分入门。从数列前缀和与区间求和讲起，类比推出定积分与有向面积，再到不定积分、原函数与积分常数，最后落到牛顿–莱布尼茨公式与微积分基本定理。全程以前缀和对应积分、差分对应微分的类比，把离散与连续两套体系串在一起，并配有能用几何图形直接算出的定积分例题。
...
\begin{aligned}
  T & =\sum_{i=a}^b A_i=A_a+A_{a+1}+\dots+A_b \\
  & =(\cancel{A_1+A_2+\dots+A_{a-1}}+A_a+A_{a+1}+\dots+A_b)-(\cancel{A_1+A_2+\dots+A_{a-1}}) \\
...
在计算定积分时，我们约定：图像在 $x$ 轴 **上方** 的区域（蓝色）面积为 **正**，**下方** 的区域（黄色）面积为 **负**，这种带符号的面积叫做 **有向面积**。
...
这个类似「前缀和」的过程叫做 **积分**，由于 $c$ 可以是任意常数，所以原函数有 **无穷多个**。
...
$$
\int_1^3\frac{x}{2}\mathrm{d}x=\left.\frac{x^2}{4}\right|_1^3=\frac{3^2}{4}-\frac{1^2}{4}=2
$$
