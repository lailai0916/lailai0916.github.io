---
title: 数学：求根公式
description: 本文将介绍一到四次方程求根公式的推导，并说明五次及以上一般方程无根式解。
summary: 从代数基本定理与韦达定理入手，依次推导一次、二次、三次、四次方程的求根公式。统一的套路是最高项归一、换元消去次高项，再借配方、拆分或平方差把问题降到低次，每一次都给出判别式与实根、复根的分类讨论。最后用伽罗瓦理论与 Abel–Ruffini 定理说明五次及以上的一般方程不存在通用的根式解。
...
「多项式」这个术语有些歧义：教科书中单项式与多项式统称为 **整式**，多项式至少要有两项。但很多时候我们讨论多项式，并不会刻意把只有一项的情形排除在外，换言之整式这个术语经常会被多项式给夺舍。
...
> [代数基本定理](https://zh.wikipedia.org/wiki/代数基本定理)：任何一个复系数多项式方程至少有一个复数根。
