---
title: 数学：大数 & 无穷
description: 本文将介绍超运算与高德纳箭号等大数记号，刻画葛立恒数与 TREE 函数，并由有限走向无穷。
summary: 从超运算讲起：把重复前一级运算这一过程不断递归，得到后继、加法、乘法、幂运算直到重幂的一整族。随后介绍高德纳箭号、康威链式箭号与阿克曼函数三种大数记号及其相互关系，再用它们刻画葛立恒数和 TREE 函数这两个常见的巨大数，最后由有限走向无穷，附带推荐读物与课程。
...
- [【漫士】这是世界上增长最快的数列，不服来战 - bilibili](https://www.bilibili.com/video/BV1cf9YBhET1)
...
4. $a\to b\to(c+1)$ 等价于 $a\to\bigl(a\to(b-1)\to(c+1)\bigr)\to c$（其中 $a\to 1\to(c+1)=a$）。
...
$$
A(m,n)=H_m(2,n+3)-3=2\uparrow^{m-2}(n+3)-3
$$
...
葛立恒数曾是以下问题中 $n$ 极其宽松的上界，但现在已经压缩到 $2\uparrow\uparrow\uparrow 6$ 了。
...
- [克鲁斯卡尔树定理 - 维基百科](https://zh.wikipedia.org/wiki/克魯斯卡爾樹定理)
