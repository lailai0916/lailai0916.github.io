---
title: 题解：P2400 秘密文件
description: 本文为洛谷 P2400 的题解，区间 DP 分拼接与折叠两类转移求最短压缩，并还原字典序最大方案。
summary: 把字符串里连续重复 k 次的子串 t 压成 k(t)，可嵌套，求最短压缩长度，多个最短方案取字典序最大者。区间 DP，设 f[i][j] 为子串压缩后的最短长度，按区间长度递推，转移分拼接（枚举断点）和折叠（判周期，长度计入重复次数的十进制位数与一对括号）两类。转移只比较整数长度，故复杂度为 O(n^3)；另用字符串数组按先短后字典序大还原最优方案。
