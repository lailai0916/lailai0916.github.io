---
title: 题解：P3812 【模板】线性基
description: 本文为洛谷 P3812 的题解，构造线性基后从高位到低位贪心异或，求一组数能得到的最大异或和。
summary: 给 n 个整数，选取任意个使异或和最大。把每个数看作 Z2 上的向量，异或即向量加法，问题化为求这组向量张成空间里数值最大的元素。构造线性基（一组线性无关且能生成整个空间的向量，按最高位所在位置存放），插入时从高位到低位消元；查询时令答案从高位起逐位尝试异或对应基向量以增大结果。文中一并给出构造、查询与贪心正确性的证明。
date: 2025-09-11T07:29:05+08:00
authors: lailai
...
给定 $n$ 个可重复的整数，选取任意个数，使得它们的异或和最大。

## 解题思路

### 思想

将每个整数视作 $\mathbb{Z}_2^k$ 中的一个 $k$ 维向量。在 $\mathbb{Z}_2$ 上，加法 $+$ 等价于按位异或 $\oplus$。

因此，问题可转化为：给定 $n$ 个向量，选择若干个相加，使结果的二进制数值尽可能大。

在线性代数中，这组向量张成一个线性空间。

我们构造它的 **线性基**（一组线性无关且能生成整个空间的最小集合），再利用线性基 **高位优先** 的结构，贪心求出最大异或值。
