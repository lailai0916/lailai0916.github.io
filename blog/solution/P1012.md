...
设 $v(s)$ 为字符串 $s$ 对应的数值，$l(s)$ 为 $s$ 的长度。

$$
\begin{aligned}
  x\succ y & \iff v(x)10^{l(y)}+v(y)>v(y)10^{l(x)}+v(x) \\
  & \iff v(x)(10^{l(y)}-1)>v(y)(10^{l(x)}-1) \\
  & \iff\frac{v(x)}{10^{l(x)}-1}>\frac{v(y)}{10^{l(y)}-1}
\end{aligned}
$$

构造映射 $\varphi:S\to\mathbb{R}$：
...
$$
x\succ y\iff\varphi(x)>\varphi(y)
$$
...
由于 $\overline{p_{i+1}p_i}>\overline{p_i p_{i+1}}$，且交换不影响前后缀的数值贡献。故 $P'$ 总数值严格增大，与 $P$ 为最优解矛盾。

因此，最优解必为按 $\succ$ 降序的排列。
