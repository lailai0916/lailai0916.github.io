...
直接二维数据结构会超时，考虑将其转化为可进行 **前缀查询** 的一维问题。

### 差分

每次询问是可差分的，区间 $[l,r]$ 可以拆分为 $[1,r]-[1,l-1]$。

这样每次询问转化为 $2$ 次前缀询问：统计区间 $[1,u]$ 中满足 $a_i\le x$ 的元素数量。

### 统计

将所有 $2m$ 次前缀询问按第一维 $u$ 排序。用 $b_i$ 表示当前第二维为 $i$ 的元素数量。

对于每个前缀询问 $(u,x)$，将所有 $j\le u$ 的 $a_i$ 加入序列 $b$，得到此时前缀 $[1,u]$ 的状态。

计算 $s=\sum_{i=1}^x b_i$ 表示满足 $a_i\le x$ 的元素数量，最后将 $s$ 贡献到对应问题的答案。
