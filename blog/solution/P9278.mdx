...
summary: 博弈论题,n 堆石子每次把第 1 堆的石子往后移,判定 Charlie 与 Dan 谁有必胜策略。倒推分析:获得某堆的先手即可获得后一堆的先手,故只要拿到第 2 堆先手即获胜;而 Charlie 开局先手,只要第 1 堆不止 1 个就能移走 v1-1 个,逼对手只剩最后一个。因此 v1 不等于 1 或 n 等于 2 时 Charlie 必胜,否则 Dan 胜。时间复杂度 O(n)。
date: 2023-05-08T23:12:06+08:00
authors: lailai
...
5. Charlie 开局是先手，只要第 $1$ 堆不止 $1$ 个，都可以移走 $v_1-1$ 个，使 Dan 只能移走最后 $1$ 个。
