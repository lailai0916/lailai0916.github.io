...
description: 本文为洛谷 UVA1303 的题解，答案为凸包周长加上半径 L 整圆的周长。
summary: 给定平面上 n 个点，求包围所有点且每个点到轮廓距离不小于 L 的最短外轮廓周长。轮廓由直线段与圆弧组成：所有线段长度之和恰为凸包周长，所有圆弧圆心角之和等于凸包外角和 2π，故圆弧总长为半径 L 的整圆周长 2πL。用 Andrew 算法或 Graham 扫描法求凸包周长，再加 2πL 即为答案。复杂度 O(n log n)。
date: 2024-12-24T20:17:49+08:00
authors: lailai
...
![](https://cloud.lailai.one/f/80qTq/solution-UVA1303.png)
