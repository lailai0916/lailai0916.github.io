...
description: 本文为洛谷 P2333 的题解，二分最小半径，分类讨论各三角形与对应扇形的交集面积并累加。
summary: 圆心固定在原点，随半径增大能看到的多边形面积单调不减，故二分最小半径使可视面积达到给定值。把多边形每个顶点与原点连成三角形，对每个三角形与对应扇形按顶点是否落在圆内、直线是否与圆相交等情形分类讨论，用叉积算三角形有向面积、用 atan2 算扇形圆心角，求出交集面积并累加得到总可视面积。
date: 2024-12-25T22:10:45+08:00
authors: lailai
