...
```cpp
#import<bits/stdc++.h>
using namespace std;using G=vector<string>;int n,i,j,k;G f(G A){for(auto&s:A)reverse(s.begin(),s.end());return A;}G r(G&A){for(i=n;i--;)for(j=i;j--;)swap(A[i][j],A[j][i]);return A=f(A);}int main(){cin>>n;G A(n),B(n);for(G*x:{&A,&B})for(auto&s:*x)cin>>s;G C=f(A);cout<<(r(A)==B?1:r(A)==B?2:r(A)==B?3:C==B?4:r(C)==B||r(C)==B||r(C)==B?5:r(A)==B?6:7);}
```
