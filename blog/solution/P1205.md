...
```cpp
#import<bits/stdc++.h>
using namespace std;using G=vector<string>;int main(){int n,i,j;cin>>n;G A(n),B(n),C;for(G*x:{&A,&B})for(auto&s:*x)cin>>s;auto f=[](G&C){for(auto&s:C)reverse(s.begin(),s.end());};auto r=[&](G&C){for(i=n;j=--i;)while(j--)swap(C[i][j],C[j][i]);f(C);return B==C;};C=A;f(C);cout<<(r(A)?1:r(A)?2:r(A)?3:C==B?4:r(C)|r(C)|r(C)?5:r(A)?6:7);}
```
