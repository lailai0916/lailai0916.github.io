...
description: 本文为洛谷 P12652 的题解，用优先队列不断取出候选集合中权值最小的节点来模拟拔树过程。
summary: 拔树游戏：每步输出当前根的权值，移除根后把它的所有子节点加入候选，再从候选中取权值最小的作为新根，直到取完。由于权值互不相同，候选集合的最小值始终与题目定义的特殊路径一致，两个过程完全等价。用优先队列维护候选集合，每个节点各入队出队一次，时间复杂度 O(nlogn)。
date: 2025-09-03T13:04:48+08:00
authors: lailai
