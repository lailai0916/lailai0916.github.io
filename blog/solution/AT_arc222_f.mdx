...
tuple<ll,ll,ll> F(tuple<ll,ll,ll> a)
{
	auto [x,y,z]=a;
	ll s=x+y+z;
	if(2*x>s)return {2*x-s,2*y,2*z};
	if(2*y>s)return {2*x,2*y-s,2*z};
	if(2*z>s)return {2*x,2*y,2*z-s};
	return {s-2*x,s-2*y,s-2*z};
}
...
bool vtx(tuple<ll,ll,ll> a)
{
	auto [x,y,z]=a;
	return (x==0)+(y==0)+(z==0)>=2;
}
ll core(tuple<ll,ll,ll> a,tuple<ll,ll,ll> b,ll s)
{
	if(a==b)return 0;
	if(vtx(a)||vtx(b))return inf;
	auto [x,y,z]=a;
	auto [u,v,w]=b;
	ll res=bsgs(x,y,u,v,s);
	if(u&&v&&w)res=min(res,bsgs(x,y,(s-u)%s,(s-v)%s,s));
	return res;
}
ll solve(tuple<ll,ll,ll> a,tuple<ll,ll,ll> b)
{
	auto &[x,y,z]=a;
	auto &[u,v,w]=b;
	if(x+y+z!=u+v+w)return -1;
	ll s=x+y+z;
	if(!s)return 0;
	ll ans=inf,cnt=0;
	while(1)
