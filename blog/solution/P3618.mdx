---
title: 题解：P3618 误会
description: 本文为洛谷 P3618 的题解，DP 统计替换方案数，用 KMP 或哈希单次判断子串是否与目标相同。
summary: 每组给字符串 s 和 t，可将 s 中任意与 t 相同的子串替换成星号，求不同的替换方案数。设 f[i] 为 s 前 i 个字符范围内的方案数：长度不足时为 1；若末尾 m 个字符等于 t，则可替换或不替换，f[i] 等于 f[i-1] 加 f[i-m]；否则只能不替换，f[i] 等于 f[i-1]。用 KMP 或 Rabin-Karp 哈希单次 O(1) 判断子串匹配，文中给出两种实现。
date: 2024-11-18T20:16:41+08:00
authors: lailai
...
2. 若 $s[i-m:i]=t$，可以选择替换 $s[i-m:i]$，但要求替换前的部分不能重叠。因此可以增加方案数 $f_{i-m}$，所以 $f_i\gets f_{i-1}+f_{i-m}$。
...
\begin{cases}
  1 & i<m \\
  f_{i-1}+f_{i-m} & s[i-m:i]=t \\
  f_{i-1} & \text{otherwise}
\end{cases}
$$

利用 **KMP 算法** 或 **Rabin–Karp 算法**（Hash）等字符串匹配算法，可以单次 $O(1)$ 判断 $s[i-m:i]=t$ 是否成立。
