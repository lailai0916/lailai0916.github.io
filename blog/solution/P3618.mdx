...
summary: 每组给出字符串 s 和 t，可把 s 中任意与 t 相同的子串替换成星号，求不同方案数。动态规划按前缀长度递推：末尾长度为 m 的子串与 t 相同时，可以选择替换或保留，方案数由前一位置与前 m 个字符的状态相加；否则沿用前一位置。子串匹配可用 KMP 或 Rabin–Karp 哈希预处理，文中给出两种实现。
