...
description: 本文为洛谷 P5698 的题解，递归模拟带循环与跳转的伪代码，求关于 n 的多项式时间复杂度并输出。
summary: 给一段带 loop、op、break、continue 的伪代码，求其时间复杂度，结果是次数不超过 20 的关于 n 的多项式。用函数 f(k) 递归模拟执行 k 次的循环，k 为 0 表示执行 n 次：遇 loop 递归求内层复杂度并累加，遇 op 累加常数或 n，遇 break 令前面语句只执行一次、continue 则跳过后续本层语句。最后倒序遍历多项式各次项，按系数与幂次的格式输出。
date: 2024-01-06T16:22:54+08:00
authors: lailai
...
- $\texttt{break <statement>}$ 或 $\texttt{continue <statement>}$
  ...
  根据循环的嵌套关系，读入并忽略后面的语句。

  而 $\texttt{break}$ 前的语句只会执行 $1$ 次，即 $k\gets1$。
