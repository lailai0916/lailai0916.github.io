...
description: 本文为洛谷 UVA11177 的题解，二分可视半径，累加三角形与扇形交集的有向面积。
summary: 以原点为观察点、给定多边形为目标，随可视半径增大能看到的面积单调不减，故二分求使可见面积不小于给定值的最小半径。将每个顶点与原点连成三角形，对三角形与其对应扇形求交集面积：叉积给出三角形有向面积，atan2 给出扇形圆心角对应的有向面积，再按顶点是否在圆内、直线与圆是否相交、交点是否落在线段内分成六种情形累加。外层二分半径，每次用 O(n) 累加各边贡献。
date: 2024-12-26T15:44:54+08:00
authors: lailai
...
随着圆的半径增大，能看到的面积单调不减，所以可以二分寻找满足条件的最小半径。
...
设 $f_1(A,B)$ 表示三角形 $\triangle OAB$ 的有向面积。
...
$$
f_2(A,B,r)=\frac{\theta r^2}{2}=\frac{\operatorname{atan2}(\overrightarrow{OA}\times\overrightarrow{OB},\overrightarrow{OA}\cdot\overrightarrow{OB})r^2}{2}
$$
...
$$
r=|\overrightarrow{OP}|=|\overrightarrow{OA}+t\overrightarrow{AB}|
$$
...
$$
a=|AB|^2,b=2\left(\overrightarrow{OA}\cdot\overrightarrow{AB}\right),c=|OA|^2-r^2
$$
...
$$
\overrightarrow{OC}=\overrightarrow{OA}+t_1\overrightarrow{AB},\overrightarrow{OD}=\overrightarrow{OA}+t_2\overrightarrow{AB}
$$
...
int main()
{
	...
	while(cin>>n&&n)
	{
		...
		cout<<"Case "<<(++_)<<": "<<fixed<<setprecision(2)<<l<<'\n';
	}
	return 0;
}
```
