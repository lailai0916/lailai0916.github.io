...
接着求每条边的选择概率。设 $u$ 的各儿子重链长度为随机变量 $L_1,\dots,L_k$（不同子树相互独立），$S=\sum L_j$，则 $P(u\to c)=\mathbb E\left[\frac{L_c}{S}\right]$。分母里的求和是难点，用积分恒等式 $\frac1S=\int_0^1 t^{S-1}\mathrm dt$ 把它拆开：记 $f_j(t)=\mathbb E[t^{L_j}]$ 为 $L_j$ 的概率生成函数，由独立性
...
而 $P(u\to c)$ 恰是上式中第 $c$ 个儿子对所有 $\ell$ 的贡献之和。其中 $\int_0^1 t^m\mathrm dt=\frac1{m+1}$，在模意义下即 $m+1$ 的逆元。
