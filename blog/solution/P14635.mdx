...
description: 本文为洛谷 P14635 的题解，把每种糖果拆成单颗装与两颗装，对购买总数二分答案求最优花费。
summary: n 种糖果库存无限，第 i 种的售价按 x_i、y_i 交替循环，求 m 元最多买多少颗。把每种糖果拆成售价 x_i、限购一组的单颗装，和售价 x_i 加 y_i、不限组数的两颗装，两类独立分别排序。花费随数量单调递增，对购买总数二分答案；算买 k 颗的最小花费时枚举两颗装组数 p，单颗装取前缀和，两颗装取最便宜的一种乘以 p。时间复杂度 O(nlogm)，需要使用 128 位整数防止溢出。
