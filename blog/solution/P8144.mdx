...
description: 本文为洛谷 P8144 的题解，用三个布尔条件判定 BBWWBB 棋局能否无限进行。
summary: 棋盘博弈判定题。数轴上按 BBWWBB 排布六枚棋子，棋子位置单调递增，问游戏能否无限进行下去。要无限进行需同时满足三个条件：白方先手；白方第一步能吃掉一个黑子，即相邻黑白棋子位置差为 1；吃子之后白子不被黑方吃回。逐一用布尔表达式判断，满足则输出 Yes 否则 No。每组数据 O(1)。
date: 2022-02-12T13:26:10+08:00
authors: lailai
