...
description: 本文为洛谷 CF1957B 的题解，从低位起尽量给第一个元素补一，余下归第二个，令按位或一最多。
summary: 构造长度为 n 的非负整数列，满足元素和为 k，使按位或结果中二进制 1 的个数最多。把第 i 位置 1 的代价为 2 的 i 次方，低位代价更小，故从低位到高位尽量给第一个元素补 1，剩余部分全放到第二个元素、其余置 0。特判 n 等于 1 时只能取 k。
date: 2024-04-22T22:02:55+08:00
authors: lailai
