---
title: 题解：CF2033F Kosuke's Sloth
description: 本文为洛谷 CF2033F 的题解，借斐波那契模 k 的皮萨诺周期，求首个被 k 整除的下标乘 n。
summary: 求斐波那契（Fibonacci）数列中第 n 个能被 k 整除的数的下标。斐波那契数列对 k 取模具有周期性（皮萨诺周期），周期 r 不超过 6k，且能被 k 整除的位置恰以 r 为间隔出现。暴力枚举求出首个被 k 整除的下标 r，则第 n 个的下标为 r 乘 n，对大质数取模输出。
date: 2024-10-25T11:00:59+08:00
authors: lailai
tags: [oi, solution, codeforces]
lid: ej0sc65a
---

{/* truncate */}

## 参考资料

- [Pisano period - Wikipedia](https://en.wikipedia.org/wiki/Pisano_period)
- [A000045 - OEIS](https://oeis.org/A000045)

## 题意简述

求 Fibonacci 数列中第 $n$ 个能被 $k$ 整除的数的下标。

## 解题思路

Fibonacci 数列对 $k$ 取模具有周期性（皮萨诺周期），且周期 $r\le 6k$。

暴力枚举求出周期 $r$，则第 $n$ 个数的下标为 $rn$。

## 参考代码

```cpp
#include <bits/stdc++.h>
using namespace std;

using ll=long long;
const int mod=1e9+7;
int main()
{
	ios::sync_with_stdio(false);
	cin.tie(nullptr);
	int T;
	cin>>T;
	while(T--)
	{
		ll n,k;
		cin>>n>>k;
		n%=mod;
		if(k==1)
		{
			cout<<n<<'\n';
			continue;
		}
		ll f1=1,f2=1,f3=1,cnt=2;
		while(f3!=0)
		{
			f3=(f1+f2)%k;
			f1=f2;
			f2=f3;
			cnt++;
		}
		cout<<cnt*n%mod<<'\n';
	}
	return 0;
}
```
