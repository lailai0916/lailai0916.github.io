---
title: '题解：P14514 [NFLSPC #8] 如何区分北京东路和北京东路'
description: 本文为洛谷 P14514 的题解，把单个位置的期望化为线性递推，套等比数列求和得到通项公式。
summary: 给定长度 n 的数列，每次随机选一个元素清零并把它平均分给其他 n 减 1 个元素，求 k 次操作后每个位置的期望，对 998244353 取模。数列总和恒为定值。对单个位置写出一步期望，化简成关于上一步的线性递推，系数为 (n-2)/(n-1)，常数项含总和，再套等比数列求和得到通项公式，用快速幂和逆元计算。时间复杂度 O(nlogk)。
date: 2025-11-19T12:12:50+08:00
authors: lailai
