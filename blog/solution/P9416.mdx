---
title: 题解：P9416 [POI 2021/2022 R1] Domino
description: 本文为洛谷 P9416 的题解，将方案数分解为斐波那契数之积，剪枝暴搜最小 n。
summary: 用 1 乘 2 与 2 乘 1 的骨牌铺满 2 乘 n 的矩形，给定铺法总数 m，求最小的 n。宽为 x 的满铺方案数是斐波那契数第 x 项，若在若干列设障碍把矩形切成几段，总方案数按乘法原理是各段方案数之积。于是问题化为把 m 分解成若干个斐波那契数之积，最小化 n（每段占若干列，段间隔一列）。因第 90 项已超过 10 的 18 次方，加剪枝后暴搜分解即可。
date: 2023-06-22T10:13:33+08:00
authors: lailai
...
![](https://cloud.lailai.one/f/JrzIn/solution-P9416.png)

若某列中，上下两行的填充方式不一致，会导致两侧剩余奇数个格子，无法完全填满。

这两种填充方式分别覆盖了 $1$ 列和 $2$ 列，如果要覆盖 $x$ 列，易得方案数为 Fibonacci 数列中第 $x$ 项 $f_x$。
