...
description: 本文为洛谷 P5656 的题解，用扩展欧几里得算法求一组解，再按通解公式求正整数解的范围与个数。
summary: 求解二元一次不定方程 ax+by=c：无整数解输出 -1；有正整数解时输出正整数解的个数以及 x、y 的最小值与最大值；有整数解但无正整数解时输出 x、y 的最小正值。先用扩展欧几里得算法求出 gcd(a,b) 和一组整数解，由裴蜀定理判无解，再把解按通解公式平移，用取模求出 x、y 的最小正值，进而得到最大值与解的个数。文中一并推导欧几里得算法、裴蜀定理与扩展欧几里得算法。
date: 2025-11-11T15:29:28+08:00
authors: lailai
...
$$
\frac{c}{g}\cdot ax_0+\frac{c}{g}\cdot by_0=\frac{c}{g}\cdot g=c
$$
