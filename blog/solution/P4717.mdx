...
description: 本文为洛谷 P4717 的题解，用快速莫比乌斯与沃尔什变换求或、与、异或卷积，正变换点乘后逆变换。
summary: 位运算卷积模板题：给两个长度为 2 的 n 次方的序列 A、B，分别在按位或、按位与、按位异或下求 C，其中 C_k 是所有 i 运算 j 等于 k 的 A_i 乘 B_j 之和。快速莫比乌斯变换处理或卷积、与卷积，本质是子集和与超集和，即 Zeta 变换与莫比乌斯反演；快速沃尔什变换处理异或、同或卷积。三者都是正变换后逐位相乘再逆变换，时间复杂度为 O(m log m)。文中给出位运算与集合的对应、变换与反演的原理推导，以及分治递归、倍增迭代、高维前缀和三种实现的代码与性能对比，并扩展到子集卷积。
date: 2025-08-06T19:00:55+08:00
authors: lailai
...
给定两个长度为 $2^n$ 的序列 $A,B$，定义：

$$
C_k=\sum_{i\oplus j=k}A_i\times B_j
$$

分别在 $\oplus$ 是 $\operatorname{or},\operatorname{and},\operatorname{xor}$ 时求序列 $C$。
...
若将 **加法**（$+$）替换为 **位运算**（$\operatorname{or},\operatorname{and},\operatorname{xor},\operatorname{xnor}$），得到 **位运算卷积** 该如何处理呢？

显然，直接枚举的时间复杂度为 $O(m^2)$，因此需要引入 FMT 和 FWT。
...
莫比乌斯反演也广泛出现于其他数学和算法领域，例如 [数论中的莫比乌斯反演](https://oi-wiki.org/math/number-theory/mobius/)。
...
$$
i\operatorname{or}j=k\implies i\subseteq k\land j\subseteq k
$$

若 $i\operatorname{and}j=k$（交集为 $k$），则 $i$ 和 $j$ 必为 $k$ 的超集：

$$
i\operatorname{and}j=k\implies i\supseteq k\land j\supseteq k
$$

我们可以构造：

$$
\operatorname{FMT_{or}}[A]_k=\sum_{i\subseteq k}A_i
$$

$$
\operatorname{FMT_{and}}[A]_k=\sum_{i\supseteq k}A_i
$$
...
显然，直接枚举的时间复杂度为 $O(m^2)$。类似 FFT，可以考虑分治。
...
扩展到 **高维前缀和**（K-Dimensional Prefix Sum），就是各维度索引都 **不超过** 它的元素之和。

$$
S_{i_1,\dots,i_n}=\sum_{j_1\le i_1,\dots,j_n\le i_n}A_{j_1,\dots,j_n}
$$

计算高维前缀和的常用方法有两种：**容斥原理** 和 **逐维前缀和**。
...
同理，可以通过 **高维后缀和**（K-Dimensional Suffix Sum）得到 **超集和**。

所以，我们也可以通过 **高维前/后缀和** 求出 Zeta 变换。
...
$$
H_{i,j}=\sum_{k=0}^i(F_k*_{\operatorname{or}}G_{i-k})_j
$$
...
注意：在模意义下，$\frac{1}{2}$ 表示 $2$ 的乘法逆元，即 $2^{-1}=\frac{p+1}{2}\pmod p$。
...
| 异或卷积（$\operatorname{xor}$）  | $939.02$ | $1167.31(+24.3\%)$ |         —         |
| 同或卷积（$\operatorname{xnor}$） | $937.16$ | $1164.39(+24.2\%)$ |         —         |
