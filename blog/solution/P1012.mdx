---
title: 题解：P1012 [NOIP 1998 提高组] 拼数
description: 本文为洛谷 P1012 的题解，按拼接后较大者在前的准则排序数字串，降序拼接即得最大整数。
summary: 给定 n 个正整数，把它们首尾相接拼成一个数，求能组成的最大整数。关键在排序的比较准则：对两个数字串 x 和 y，若拼接 xy 大于拼接 yx 则 x 应排在前。可证该关系构成严格弱序，且交换任意相邻逆序对都会使结果严格增大，因此按此准则降序排列即为最优。时间复杂度为 O(n log n) 级别。
date: 2026-01-17T13:39:05+08:00
authors: lailai
