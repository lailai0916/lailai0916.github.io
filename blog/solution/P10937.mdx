...
description: 本文为洛谷 P10937 的题解，行列视为二分图两侧，可放位置连边，匈牙利算法求最大匹配即最多車数。
summary: n 乘 m 的矩阵中给定若干不可放置的位置，要求每行每列至多放一个車，求最多能放多少个。把每一行与每一列各看作二分图一侧的节点，可放的位置在对应行与列之间连一条边，则答案就是这张二分图的最大匹配，用匈牙利算法求出即可。
date: 2024-10-30T16:09:10+08:00
authors: lailai
