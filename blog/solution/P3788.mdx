...
description: 本文为洛谷 P3788 的题解，一道几何题，按两切角关系分类讨论各半圆对红色与可见面积的贡献。
summary: 一道几何题，西瓜被切去一段后求正视时红色部分占可见面积的比例。设截面半圆面积为 1，角度为 x 的半圆投影面积为 sin x。图形左右对称且独立，按两切角的大小关系分类讨论每个半圆对总可见面积与红色可见面积的贡献，把整圆拆成两个半圆、后半圆用对称映射到前半圆处理，最后输出红色面积与总可见面积之比。
