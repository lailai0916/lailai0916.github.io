...
其中，$\operatorname{merge}$ 表示将两个序列像字符串拼接一样首尾相连，加号（$+$）就是直接逐项相加。
...
$$
S_{i_1,\cdots,i_n}=\sum_{T\subseteq \{1,\cdots,n\}}(-1)^{|T|}A_T
$$
...
第一个限制 $i\operatorname{or} j=k$，很容易处理，直接用 **或卷积** 处理即可。
...
$$
H_{i,j}=\sum_{k=1}^i(F_{k}*_{\operatorname{or}}G_{i-k})_j
$$
