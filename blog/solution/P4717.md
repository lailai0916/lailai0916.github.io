...
这个变换称为 **Zeta 变换**，即子集/超集前缀和。

#### Möbius 变换

而 Zeta 的逆变换称为 **Möbius 变换**。
...
容易发现顺变换和逆变换可以合并为一个函数。顺变换时 $\text{type}=1$，逆变换时 $\text{type}=-1$。
...
void fmt_or(ll *a,int n,int type)
{
	...
	for(int i=0;i<mid;i++)a[mid+i]=(a[mid+i]+a[i]*type+mod)%mod;
}
```
