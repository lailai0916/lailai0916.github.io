...
快速傅立叶变换（Fast Fourier Transform，FFT）可以 $O(n\log n)$ 计算 **加法卷积**：
...
与 FFT 类似，FMT/FWT 首先 $O(n\log n)$ 进行 **正变换**：

$$
A\to\operatorname{FWT}[A]
$$

$$
B\to\operatorname{FWT}[B]
$$
...
$$
C\gets\operatorname{FWT}[C]
$$

快速莫比乌斯变换（Fast Möbius Transform，FMT）可以 $O(n\log n)$ 求解：
...
快速沃尔什变换（Fast Walsh Transform，FWT）可以 $O(n\log n)$ 求解：
