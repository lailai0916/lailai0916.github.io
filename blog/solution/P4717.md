...
综上，我们可以写出 `fmt_or` 和 `fmt_and` 的递归版本：
...
类似 FFT，我们可以写出 `fmt_or` 和 `fmt_and` 更高效（常数更小）的迭代版本：
...
- 数据规模：$2^{24}=16,777,216\approx‌ 1.68\times 10^7$ 随机整数
- 测试方法：每个算法连续运行 $100$ 次，记录耗时并计算平均值

| 算法 | 或卷积（$\operatorname{or}$） | 与卷积（$\operatorname{and}$） | 异或卷积（$\operatorname{xor}$） |
| :-: | :-: | :-: | :-: |
| 递归实现 | $665.67\mathrm{ms}$ | $693.12\mathrm{ms}$ | $1167.31\mathrm{ms}$ |
| 迭代实现 | $575.31\mathrm{ms}$ | $559.65\mathrm{ms}$ | $939.02\mathrm{ms}$ |
