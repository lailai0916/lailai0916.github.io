...
description: 本文为洛谷 AT_arc168_a 的题解，按相邻大小贪心构造逆序对最少的序列，再用归并排序统计。
summary: 给定长度 n 减 1 的大小关系序列（每位是大于或小于号），构造一个满足这些关系、逆序对尽量少的序列并求其逆序对数。贪心地让后面的数尽可能大：遇到大于号取前一项减 1，遇到小于号取前一项加一个足够大的量，最后用归并排序统计逆序对。时间复杂度 O(nlogn)。
date: 2023-11-28T21:25:21+08:00
authors: lailai
