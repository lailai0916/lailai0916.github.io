...
description: 本文为洛谷 P6307 的题解，化简单点阵的正三角形概率，用 Digamma 函数把无穷和写成闭式。
summary: 在三角形点阵中随机取三个点，求它们构成正三角形的概率，并对点阵大小从 m 到无穷求和。先数出大小为 i 的三角形框贡献 i-1 个正三角形、可放在 a(k-i+1) 个位置，化简得单个点阵的概率 P_k 等于 2 除以 k 方加 k 减 4。对分母裂项，用 Digamma 函数把无穷和写成闭式，代码里用递推关系加渐近展开式求 Digamma 的近似值。
date: 2024-11-21T16:01:32+08:00
authors: lailai
...
![](https://cloud.lailai.one/f/K8bcJ/solution-P6307.png)
...
\begin{aligned}
  ...
  & =\frac{24(-k^3-3k^2-2k+\sum_{i=1}^ki^3-(2k+4)\sum_{i=1}^ki^2+(k^2+5k+5)\sum_{i=1}^ki}{k(k+1)(k^2+k-2)(k^2+k-4)} \\
