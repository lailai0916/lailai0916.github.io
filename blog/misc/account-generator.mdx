---
title: 项目：账号生成器
description: 本文将介绍一个用 C++ 编写的账号生成器，随机生成用户名并由其哈希复现对应密码。
summary: 一个用来随机生成账号的 C++ 小程序。它从内置的英文名与姓氏词库中各取一个，首字母大写后拼上一个四位随机数，截断为不超过 16 位的用户名；再以该用户名的哈希值作为随机种子，从大小写字母与数字中取 12 个字符生成对应密码，做到同名同密、可以复现。
date: 2025-10-21T20:21:00+08:00
authors: lailai
tags: [misc]
---
