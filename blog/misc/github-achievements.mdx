---
title: 项目：GitHub 成就工具
description: 本文将介绍一组使用 Playwright 自动获取 GitHub 成就徽章的脚本与运行方法。
summary: 用 Playwright 自动刷取 GitHub 个人主页 Achievements 徽章的一组脚本。先用一个登录脚本打开浏览器并保存会话状态，之后 Pull Shark 脚本循环建分支、改 README、提交并合并 Pull Request，Galaxy Brain 脚本则用两个账号分别新建讨论、回复并标记为答案，从而批量累积对应成就。
date: 2025-07-17T15:48:00+08:00
authors: lailai
tags: [misc]
---
...
async function loginWithAccount(accountName) {
  ...
  console.log(`正在为账号 ${accountName} 启动登录流程...`);
  ...
  console.log(`请在浏览器中操作，60 秒后自动保存 ${accountName} 的登录状态`);
...
// 主程序
(async () => {
  // 从命令行参数获取账号名，默认为 account1
  const accountName = process.argv[2] || 'account1';

  // 验证账号名格式
  if (!accountName.match(/^account\d+$/)) {
    console.error('错误：账号名格式应为 accountN（如 account1, account2）');
    process.exit(1);
  }
...
async function autoCreatePullRequest(page, k) {
  ...
  console.log('输入分支名');
  await page.waitForSelector('input[placeholder="Find or create a branch..."]', { timeout: 5000 });
  await page.fill('input[placeholder="Find or create a branch..."]', branchName);
  await page.waitForTimeout(1000);
  console.log('点击创建分支按钮');
  await page.waitForSelector(`button:has-text("Create branch ${branchName} from main")`, {
    timeout: 5000,
  });
  ...
  console.log('点击 Commit changes');
  await page.waitForSelector('div[role="dialog"] >> button:has-text("Commit changes")', {
    timeout: 5000,
  });
  await page.waitForTimeout(3000);
  const confirmButton = page.locator('div[role="dialog"] >> button:has-text("Commit changes")');
