...
- [狄利克雷生成函数 - OI Wiki](https://oi.wiki/math/poly/dgf/#dirichlet-卷积)
- [筛法 - OI Wiki](https://oi.wiki/math/number-theory/sieve/#筛法求约数和)
...
时间复杂度 $O(T+n\log\log{n})$，实际用时 $6.2\mathrm{s}$。
...
const int N=100000005;
int prime[N],spf[N],sigma[N],ans[N];
bool vis[N];
void init()
{
	vis[0]=vis[1]=1;
	...
	for(int i=2;i<N;i++)
	{
		...
		for(int j=1;j<=cnt;j++)
		{
			...
			if(i%prime[j]==0)
			{
				spf[i*prime[j]]=spf[i]*prime[j]+1;
				sigma[i*prime[j]]=sigma[i]/spf[i]*spf[i*prime[j]];
				break;
			}
			spf[i*prime[j]]=prime[j]+1;
